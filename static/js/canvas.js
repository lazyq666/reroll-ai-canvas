...
function actionFailed(labelKey, detail=''){
    ...
    const separator = detail ? (langIsEn() ? ': ' : '：') : '';
    return trf('canvas.actionFailed', {action: label, detail: `${separator}${detail}`});
}
function noReturnedImage(labelKey){ return trf('canvas.noImageReturned', {action: tr(labelKey)}); }
...
function canvasProxiedMediaUrl(url, name=''){
    ...
    if(!raw || raw.startsWith('/assets/') || raw.startsWith('data:') || raw.startsWith('blob:')) return raw;
...
function canvasMediaPreviewUrl(url, size=512){
    ...
    if(!raw.startsWith('/assets/')) return canvasDisplayMediaUrl(raw);
...
const MS_GEN_MODELS = {
    ...
    custom:    { label: '', labelKey: 'canvas.custom', modelId: '',                acceptsImage: true,   endpoint: '/api/ms/generate'     }
};
...
let canvasPromptTemplateSaving = false;
let canvasPromptCommitLane = null;
...
function applyQuickToolbarState(){
    ...
    if(btn){
        btn.title = tr(collapsed ? 'canvas.expandQuickMenu' : 'canvas.collapseQuickMenu');
...
function catalogEntries(kind){
    ...
    return (apiProviders.length ? apiProviders : defaultApiProviders())
        ...
        .flatMap(provider => uniqueModels(provider[field] || []).map(model => ({
...
function catalogEntry(kind, providerId='', model=''){
    const entries = catalogEntries(kind);
    return entries.find(entry => entry.provider_id === providerId && entry.model === model)
        || entries.find(entry => entry.model === model)
        || entries[0]
        || null;
}
function catalogEntryById(kind, id){
    return catalogEntries(kind).find(entry => entry.id === id) || null;
}
function catalogModelOptions(kind, providerId='', model=''){
    const entries = catalogEntries(kind);
    if(!entries.length) return `<option value="" disabled selected>${tr('canvas.noModelsContactAdmin')}</option>`;
    const selected = catalogEntry(kind, providerId, model);
    return entries.map(entry => `<option value="${escapeHtml(entry.id)}" ${entry.id === selected?.id ? 'selected' : ''}>${escapeHtml(entry.name || entry.model)} · ${escapeHtml(entry.provider_name || entry.provider_id)}</option>`).join('');
}
function catalogPlatformName(kind, providerId='', model=''){
    const entry = catalogEntry(kind, providerId, model);
    return entry?.provider_name || entry?.provider_id || '';
}
function canvasModelVendorIconMarkup(kind, providerId='', model=''){
    const entry = catalogEntry(kind, providerId, model);
    return window.ModelVendorIcons?.markup(
        entry?.model || model,
        entry?.provider_id || providerId,
        entry?.provider_name || ''
    ) || '';
}
function syncCanvasModelVendorIcon(container, entry){
    const current = container?.querySelector('.model-vendor-icon');
    if(!current) return;
    const template = document.createElement('template');
    template.innerHTML = window.ModelVendorIcons?.markup(entry?.model, entry?.provider_id, entry?.provider_name) || '';
...
function providerOptions(selectedId){
    ...
    const providers = imageApiProviders();
    if(!providers.length) return `<option value="" disabled selected>${tr('canvas.noApiProviders')}</option>`;
...
function sanitizeImageNodeProviderModel(node){
    ...
    const entry = catalogEntry('image', node.apiProvider || '', resolveImageModel(node.model));
    node.apiProvider = entry?.provider_id || '';
...
function sanitizeVideoNodeProviderModel(node){
    ...
    const entry = catalogEntry('video', node.apiProvider || '', node.model || '');
    node.apiProvider = entry?.provider_id || '';
    node.model = entry?.model || '';
}
function videoModelOptions(selectedModel, providerId){
    return catalogModelOptions('video', providerId, selectedModel);
}
...
function imageModelOptions(selectedModel, providerId){
    return catalogModelOptions('image', providerId, resolveImageModel(selectedModel));
}
function chatModelOptions(selectedModel, providerId=''){
    return catalogModelOptions('text', providerId, selectedModel);
}
...
function setCreateMode(active, kind='smart'){
    ...
    createCanvasKind = active ? ((kind === 'smart') ? 'smart' : 'classic') : 'smart';
...
async function saveCanvas(){
    ...
    try {
        ...
        setStatus(tr('canvas.saved'));
        loadCanvasList(false);
    } catch(e) {
        setStatus(tr('canvas.saveFailed'));
        console.error(e);
    } finally {
...
async function classicCanvasCheckpoint({timeout=5000}={}){
    ...
    for(;;){
        ...
        if(
            ...
            return Object.freeze({revision:Number(canvas?.revision || 0)});
        }
        if(Date.now() >= deadline){
            const error = new Error(tr('canvas.canvasStillSaving'));
...
async function classicCanvasPromptResync(){
    await classicCanvasCheckpoint({timeout:5000});
    const response = await fetch(`/api/canvases/${encodeURIComponent(canvas.id)}`);
    if(!response.ok) return false;
    const data = await response.json().catch(() => ({}));
    if(!data.canvas || localCanvasDirty || savingCanvasNow || saveCanvasAgain) return false;
...
try {
    ...
    apiChannel.onmessage = async (e) => {
        if(e.data?.type === 'providers-changed' || e.data?.type === 'models-changed' || e.data?.type === 'workflows-changed' || e.data?.type === 'comfy-instances-changed'){
...
function msChatModelOptions(selected){
    ...
    if(!list.length){
        return `<option value="" disabled selected>${tr('canvas.noModelsHint')}</option>`;
    }
...
async function patchCanvasMeta(id, patch){
    ...
    try {
        ...
        setStatus(tr('canvas.metaSaveFailed'));
        console.error(e);
        ...
        row.innerHTML = `
            <div class="canvas-open" role="button" tabindex="${trashMode ? '-1' : '0'}">
                <div class="canvas-card-icon-row">
                    <span class="canvas-preview-mark ${color ? `icon-has-color cc-${escapeAttr(color)}` : ''}" role="button" tabindex="0" title="${trashMode ? tr('canvas.deletedCanvas') : tr('canvas.editMeta')}">${renderCanvasIcon(isSmartCanvas && /[^\x00-\x7F]/.test(item.icon || '') ? 'sparkles' : item.icon, 16)}</span>
            ...
            ` : `
                <button class="canvas-pin-btn ${pinned ? 'active' : ''}" type="button" title="${pinned ? tr('canvas.unpin') : tr('canvas.pin')}" aria-label="${pinned ? tr('canvas.unpin') : tr('canvas.pin')}">
...
function renderCanvasMetaPopover(){
    ...
    pop.innerHTML = `
        <div class="canvas-meta-section">
            <div class="canvas-meta-label">${tr('canvas.ownerLabel')}</div>
            <input class="canvas-owner-input" type="text" maxlength="40" value="${escapeAttr(owner)}" placeholder="${escapeAttr(tr('canvas.ownerPlaceholder'))}">
        </div>
        <div class="canvas-meta-section">
            <div class="canvas-meta-label">${tr('canvas.colorLabel')}</div>
            <div class="canvas-color-row">
                <button class="canvas-color-swatch cc-none ${!color ? 'active' : ''}" type="button" data-color="" title="${tr('canvas.colorNone')}"><i data-lucide="ban" class="w-3 h-3"></i></button>
...
async function setCanvasIcon(id, icon, event){
    ...
    try {
        ...
        if(!res.ok) throw new Error(tr('canvas.iconSaveFailed'));
        if(canvas?.id === id) canvas.icon = target.icon;
        await loadCanvasList(false);
    } catch(e) {
        setStatus(tr('canvas.iconSaveFailed'));
        console.error(e);
    }
}
...
async function setCanvasTitle(id, title){
    ...
    try {
        ...
        if(!res.ok) throw new Error(tr('canvas.renameFailed'));
        if(currentCanvasTitle && canvas?.id === id) currentCanvasTitle.textContent = title;
        await loadCanvasList(false);
    } catch(e){
        setStatus(tr('canvas.renameFailed'));
        console.error(e);
    }
}
async function openCanvas(id){
    setStatus(tr('canvas.opening'));
    try {
        const res = await fetch(`/api/canvases/${id}`);
...
function canvasLocalAssetUrls(){
    ...
    const add = value => {
        ...
        if(url && (url.startsWith('/assets/'))) urls.add(url);
    };
    nodes.forEach(node => {
...
async function syncRemoteCanvasNow(){
    ...
    try {
        ...
        console.error(e);
        setStatus(tr('canvas.syncFailed'));
    }
}
...
async function deleteCanvas(id, event){
    event?.preventDefault();
    event?.stopPropagation();
    setStatus(tr('canvas.movingToTrash'));
    try {
...
function addImageNode(point){
    ...
    return addNode({id:uid('img'), type:'image', x:p.x, y:p.y, url:'', name:tr('canvas.blankImage')});
}
...
function addLLMNode(point){
    ...
    const entry = catalogEntries('text')[0];
    const providerId = entry?.provider_id || chatApiProviders()[0]?.id || 'comfly';
    return addNode({
        id:uid('llm'),
        type:'llm',
        x:p.x,
        ...
        model:entry?.model || resolveChatModel('', providerId),
        mode:'node',
...
function addGeneratorNode(point){
    const p = point || defaultPoint(120, 0);
    const entry = catalogEntries('image')[0];
    const providerId = entry?.provider_id || imageApiProviders()[0]?.id || '';
    const model = entry?.model || allImageModels(providerId)[0] || '';
...
function addVideoNode(point){
    ...
    const entry = catalogEntries('video')[0];
    const providerId = entry?.provider_id || videoApiProviders()[0]?.id || 'comfly';
    ...
    return addNode({
        ...
        model:entry?.model || models[0] || videoModels[0] || DEFAULT_VIDEO_MODELS[0],
        duration:5,
        aspectRatio:'16:9',
        resolution:'',
        enhancePrompt:false,
...
async function getImageDimensions(url){
    return new Promise((resolve, reject) => {
        ...
        img.onerror = () => reject(new Error(tr('canvas.imageLoadFailed')));
        img.src = url;
    });
}
async function urlToBase64(url){
    const res = await fetch(url);
    if(!res.ok) throw new Error(tr('canvas.imageReadFailed'));
        ...
        <div class="ms-controls">
            <div class="gen-settings">
                ...
                <div class="gen-settings-row ms-custom-size-row" style="display:none">
                    ...
                    <button class="secondary-btn ms-fit-size-btn" type="button" style="height:var(--ui-control-height-s);align-self:flex-end;padding:var(--ui-space-0) var(--ui-space-2);font-size:var(--ui-font-size-2)">${tr('canvas.fitImageSize')}</button>
                </div>
                ${msLoras.length ? `
                <div class="gen-settings-row">
                    <label class="setting-check" style="cursor:pointer">
                        <input type="checkbox" class="ms-lora-check" ${node.msLoraEnabled ? 'checked' : ''}>
                        <span style="font-size:var(--ui-font-size-2);font-weight:var(--ui-font-weight-bold)">${tr('canvas.enableLora')}</span>
                    </label>
                </div>
                ${node.msLoraEnabled ? `
                <div class="gen-settings-row">
                ...
                ${!msLoras.length ? `<div class="gen-settings-row"><div style="color:var(--ui-color-text-tertiary);font-size:var(--ui-font-size-2);font-weight:var(--ui-font-weight-bold);line-height:var(--ui-line-height-code)">${tr('canvas.noLoraForModel')}</div></div>` : ''}
            </div>
...
function openGeneratorNodeMenu(nodeId, clientX, clientY){
    ...
    const buttonsHtml = (options, kind) => `<div class="node-port-menu-grid">${options.map(opt => `<button class="menu-btn" data-link-create="${escapeAttr(opt.type)}" data-link-kind="${kind}" title="${escapeAttr(opt.label)}"><i data-lucide="${escapeAttr(opt.icon)}"></i><span>${escapeHtml(opt.label)}</span></button>`).join('')}</div>`;
    ...
    nodeInputMenu.innerHTML = `<div class="menu-section-title">${tr('canvas.addInput')}</div>${buttonsHtml(inputOptions, 'in')}`;
    nodeOutputMenu.innerHTML = `<div class="menu-section-title">${tr('canvas.addOutput')}</div>${buttonsHtml(outputOptions, 'out')}`;
...
function openImageNodeMenu(nodeId, clientX, clientY){
    ...
    imageNodeMenu.innerHTML = `
        ${canPreview ? `<button class="menu-btn" data-image-preview="${escapeAttr(nodeId)}"><i data-lucide="eye" class="w-4 h-4"></i><span>${tr('canvas.preview')}</span></button>` : ''}
        ${canEdit ? `<button class="menu-btn" data-image-edit="${escapeAttr(nodeId)}"><i data-lucide="pencil" class="w-4 h-4"></i><span>${tr('canvas.edit')}</span></button>` : ''}
        <button class="menu-btn" data-image-replace="${escapeAttr(nodeId)}"><i data-lucide="image-plus" class="w-4 h-4"></i><span>${tr('canvas.replace')}</span></button>
    `;
...
function outputDownloadableImageUrls(node){
    return (node?.images || []).map(outputUrlValue).filter(url => url && !isMissingAssetUrl(url) && (url.startsWith('/assets/')));
}
...
async function uploadCanvasMediaRefToCloud(node, ref){
    const kind = mediaKindForRef(ref);
    if(!ref?.url) throw new Error(tr('canvas.noUploadableMedia'));
    ...
    if(!response.ok) throw new Error(await responseErrorMessage(response, tr('canvas.cloudUploadFailed')));
    const data = await response.json();
    const uploadedUrl = data.url || '';
    if(!uploadedUrl) throw new Error(tr('canvas.cloudNoLink'));
    node.tempShLinks = [
...
async function uploadCanvasVideosToCloud(nodeId){
    ...
    if(!localRefs.length){
        showErrorModal(tr('canvas.noLocalMedia'), tr('canvas.cloudUpload'));
        return [];
    }
    ...
    try {
        ...
        showErrorModal(trf('canvas.cloudUploadDone', {count:urls.length}), tr('canvas.cloudUpload'));
        return urls;
    } catch(e) {
        node.tempShUploading = false;
        refreshNodes([node.id]);
...
async function setCanvasManualVideoUrl(nodeId){
    ...
    const value = prompt(tr('canvas.mediaUrlPrompt'), isRemoteVideoReferenceUrl(current) ? current : '');
    ...
    if(!url){
        ...
        refreshNodes([node.id]);
        scheduleSave();
        showErrorModal(tr('canvas.urlCleared'), tr('canvas.inputUrl'));
        return '';
    }
    if(!isRemoteVideoReferenceUrl(url)){
        showErrorModal(tr('canvas.invalidMediaUrl'), tr('canvas.inputUrl'));
        return '';
    }
    applyManualVideoUrlToCanvasRef(node, firstAny, url);
    refreshNodes([node.id, firstAny?.nodeId].filter(Boolean));
    scheduleSave();
    showErrorModal(tr('canvas.videoUrlSet'), tr('canvas.inputUrl'));
    return url;
}
...
async function importLocalImages(paths){
    ...
    if(!response.ok) throw new Error(await responseErrorMessage(response, tr('canvas.localImageImportFailed')));
    const data = await response.json();
    return data.files || [];
}
...
async function createImageCardsFromLocalPaths(paths, point){
    if(!ensureCanvas()) return [];
    setStatus(tr('canvas.importingImages'));
    try {
...
async function handleImageNodeDropEvent(e, nodeId, highlightEl){
    ...
    try {
        ...
        setStatus(tr('canvas.ready'));
        showErrorModal(err.message || tr('canvas.imageImportFailed'), tr('canvas.imageImportFailed'));
    }
}
...
function clearImageNode(nodeId, event=null){
    ...
    node.url = '';
    node.mediaKind = 'image';
    node.name = tr('canvas.blankImage');
    render();
    scheduleSave();
}
...
function defaultEditTextText(){
    return tr('canvas.doubleClickEdit');
}
...
function setImageEditMode(mode, userTouched=false){
    ...
    if(isPreview){
        ...
        if(imageEditMode === 'resize'){
            title.textContent = tr('canvas.resizeImage');
            sub.textContent = tr('canvas.resizeImageSub');
            apply.innerHTML = `<i data-lucide="minimize-2" class="w-4 h-4"></i><span>${tr('canvas.applyResize')}</span>`;
        } else {
...
async function applyImageGridSplit(){
    ...
    if(files.length){
        ...
        appendOutputImages(out, urls, {url:node.url, name:node.name || 'source image'}, urls.map((url, i) => ({
            runMs:0,
            run:{prompt:tr('canvas.modeGrid'), refs:[{url:node.url, name:node.name || 'source image'}]},
...
function renderPendingOutput(pending){
    if(pending?.failed){
        ...
        const sub = taskId ? trf('canvas.taskId', {id:escapeHtml(taskId)}) : tr('canvas.noTaskIdQuery');
        return `<div class="output-img-wrap loading-wrap recoverable" data-pending-id="${escapeAttr(pending.id)}"${pendingOutputStyle(pending)}>
            <span class="output-time-pill failed">${tr('canvas.failed')}</span>
            <div class="output-recover-state">
                <i data-lucide="refresh-cw" class="${querying ? 'spinning' : ''}"></i>
                <div class="output-recover-title">${tr(querying ? 'canvas.querying' : 'canvas.taskRecoverable')}</div>
                <div class="output-recover-sub" title="${escapeAttr(msg)}">${sub}</div>
                <button class="output-recover-query" type="button" ${taskId && !querying ? '' : 'disabled'}>${tr(querying ? 'canvas.queryingEllipsis' : 'canvas.queryResult')}</button>
            </div>
...
function renderNode(node){
    ...
    const title = node.type === 'image' ? tr('canvas.image') : node.type === 'prompt' ? tr('canvas.prompt') : node.type === 'loop' ? tr('canvas.loopNode') : node.type === 'promptGroup' ? tr('canvas.promptGroup') : node.type === 'group' ? tr('canvas.group') : node.type === 'output' ? tr('canvas.outputNode') : node.type === 'llm' ? 'LLM' : node.type === 'comfy' ? 'ComfyUI' : node.type === 'ltxDirector' ? tr('canvas.ltxDirector') : node.type === 'rh' ? 'RunningHub' : node.type === 'msgen' ? tr('canvas.modelscopeGenerate') : node.type === 'video' ? tr('canvas.videoGenerateNode') : tr('canvas.apiGenerate');
    ...
    const statusHtml = showStatus ? (() => {
        const statusKey = { queued:'canvas.queued', running:'common.running', done:'canvas.done', failed:'canvas.failed' }[node.runStatus];
        const label = statusKey ? tr(statusKey) : '';
    ...
    el.innerHTML = `<div class="node-head"><span class="node-title">${displayTitle}</span><div style="display:flex;align-items:center;gap:var(--ui-space-2)">${statusHtml}<button type="button" class="node-delete-button text-gray-300 hover:text-red-500"><i data-lucide="x" class="w-4 h-4"></i></button></div></div>`;
    const deleteButton = el.querySelector('.node-delete-button');
    ...
    if(node.type === 'image') {
        if(node.url) {
            ...
            body.innerHTML = `<div class="image-preview-wrap">${missing ? missingAssetHtml(node.url) : canvasPreviewImgHtml(node.url, 768, 'draggable="false"')}</div><div class="image-caption text-[11px] text-gray-400 truncate">${escapeHtml(node.name || 'image')}${missing ? ` · ${tr('canvas.missingFile')}` : ''}</div>`;
            if(!missing && mediaKind !== 'image'){
                const mediaHtml = mediaKind === 'video'
                    ? `<div class="media-card video-card">${canvasVideoPreviewHtml(node.url, 768, 'draggable="false" data-video-fallback-attrs="controls"')}<button class="canvas-video-play" type="button" title="${tr('canvas.play')}"><i data-lucide="play"></i></button></div>`
...
function bindOutputWrap(wrap, node){
    ...
    if(fileCard){
        fileCard.onclick = e => {
            ...
            if(url) downloadUrl(url, outputDownloadName(url)).catch(err => alert(err.message || tr('canvas.downloadFailed')));
        };
    }
    if(del){
...
// Prompt Library v2: Classic Canvas shares the same controlled component and
// scope contract as Smart Canvas. The shared module owns every visible library,
// editor, and confirmation state; this host only adapts persistence and Canvas use.
...
function syncCanvasPromptTemplateButtons(){
    const activeId = promptTemplateDialog?.open ? promptTemplateNodeId : '';
...
function canvasPromptTemplateDefaultName(text){
    return (String(text || '').trim().split(/\r?\n/)[0] || tr('canvas.newPrompt')).slice(0, 28);
}
function classicPromptNotify(message, options={}){
    const notify = () => {
        const Toast = customElements.get('ic-toast');
        if(typeof Toast?.notify !== 'function') return false;
        Toast.notify(String(message || ''), {
...
function classicCommonPromptLibrary(){
    return canvasPromptLibraries.find(library => library.id === 'common') || {id:'common', name:tr('canvas.commonLibrary'), scope:'common', categories:[], items:[]};
}
async function loadCanvasPromptTemplates(){
    const [commonResponse, canvasResponse] = await Promise.all([
        fetch('/api/prompt-libraries'),
        fetch(`/api/canvases/${encodeURIComponent(canvas.id)}/prompt-templates`)
    ]);
    ...
    const common = commonData.library?.common || {id:'common', name:tr('canvas.commonLibrary'), scope:'common', categories:[], items:[]};
    if(canvas) canvas.revision = Math.max(Number(canvas.revision || 0), Number(canvasData.revision || 0));
    canvasPromptLibraries = [
        {...common, id:'common', scope:'common', readonly:false, items:(common.items || []).map(item => ({...item, libraryId:'common', remote:true, builtin:false}))},
        {id:'canvas', name:tr('canvas.currentCanvas'), scope:'canvas', readonly:false, categories:[], items:(canvasData.templates || []).map(item => ({...item, libraryId:'canvas', remote:true, builtin:false}))}
    ];
    canvasPromptTemplatesLoaded = true;
    if(!canvasPromptLibraries.some(library => library.id === activePromptLibraryId)) activePromptLibraryId = 'canvas';
    canvasPromptTemplates = activeCanvasPromptLibraryItems();
    return canvasPromptTemplates;
}
function activeCanvasPromptLibrary(){
    return canvasPromptLibraries.find(library => library.id === activePromptLibraryId)
        || canvasPromptLibraries.find(library => library.id === 'canvas')
        || {id:'canvas', name:tr('canvas.currentCanvas'), scope:'canvas', readonly:false, categories:[], items:[]};
}
function activeCanvasPromptLibraryItems(){
    ...
    return (library.items || [])
        ...
        .map(item => ({...item, libraryId:library.id, remote:true, builtin:false}));
}
function allCanvasPromptLibraryItems(){
...
function activeCanvasPromptTemplateGroups(){
    const library = activeCanvasPromptLibrary();
    return library.id === 'common' && Array.isArray(library.categories) ? library.categories : [];
}
function canvasPromptTemplateCategoryLabel(category){
    if(category === 'all') return tr('smart.tplAll');
    return activeCanvasPromptTemplateGroups().find(group => group.id === category)?.name || '';
}
function currentCanvasPromptTemplateLibraryEditable(){
    const library = activeCanvasPromptLibrary();
    return Boolean(library && !library.readonly);
}
function selectedCanvasPromptTemplate(){
    return activeCanvasPromptLibraryItems().find(item => item.id === promptTemplateSelectedId) || null;
}
function applyClassicCanvasPromptResponse(data={}){
    if(canvas) canvas.revision = Math.max(Number(canvas.revision || 0), Number(data.revision || 0));
    const library = canvasPromptLibraries.find(item => item.id === 'canvas');
    if(library && Array.isArray(data.templates)) library.items = data.templates.map(item => ({...item, libraryId:'canvas', remote:true, builtin:false}));
}
function classicPromptCommitLane(){
    if(canvasPromptCommitLane) return canvasPromptCommitLane;
    const module = window.InfiniteCanvasModules?.CanvasCommitLane;
    if(!module) throw new Error('Canvas Commit Lane failed to load');
    canvasPromptCommitLane = module.create({
        canvasId:() => canvas?.id || '',
        clientId:() => CLIENT_ID,
        checkpoint:() => classicCanvasCheckpoint({timeout:5000}),
        resync:classicCanvasPromptResync,
        observeExternalCommit:({revision,updatedAt}) => {
            if(canvas){
                canvas.revision = Math.max(Number(canvas.revision || 0),Number(revision || 0));
                canvas.updated_at = Math.max(Number(canvas.updated_at || 0),Number(updatedAt || 0));
                lastCanvasUpdatedAt = Math.max(Number(lastCanvasUpdatedAt || 0),Number(updatedAt || 0));
            }
            return true;
        },
        onPromptState:applyClassicCanvasPromptResponse,
    });
    return canvasPromptCommitLane;
}
function applyClassicCommonPromptResponse(data={}){
    ...
    const index = canvasPromptLibraries.findIndex(library => library.id === 'common');
    if(index >= 0) canvasPromptLibraries[index] = record;
    else canvasPromptLibraries.unshift(record);
}
function renderPromptTemplateModal(){
    if(!promptTemplatePanel) return;
    canvasPromptTemplates = activeCanvasPromptLibraryItems();
    promptTemplatePanel.libraries = canvasPromptLibraries;
    promptTemplatePanel.templates = allCanvasPromptLibraryItems();
    promptTemplatePanel.activeLibrary = activePromptLibraryId;
    promptTemplatePanel.activeCategory = activePromptLibraryId === 'common' ? promptTemplateCategory : 'all';
    promptTemplatePanel.selectedTemplate = promptTemplateSelectedId;
    promptTemplatePanel.canManage = true;
    promptTemplatePanel.busy = canvasPromptTemplateSaving;
}
async function openPromptTemplateModal(nodeId){
    ...
    if(promptTemplatePanel) promptTemplatePanel.query = '';
    try {
        await loadCanvasPromptTemplates();
    } catch(error) {
        setStatus(error.message || tr('canvas.loadFailed'));
        return;
    }
...
async function closePromptTemplateModal(event=null){
    if(event?.type !== 'ic-hide' && promptTemplateDialog?.open){
...
function applyPromptTemplateToPromptNode(mode='positive'){
    const template = activeCanvasPromptLibraryItems().find(item => item.id === promptTemplateSelectedId);
    const node = nodes.find(item => item.id === promptTemplateNodeId && item.type === 'prompt');
...
async function uploadClassicPromptCover(file){
    if(!file) return '';
    const form = new FormData();
    form.append('file', file, file.name || 'prompt-cover');
    const response = await fetch('/api/prompt-libraries/covers', {method:'POST', body:form});
    const data = await response.json().catch(() => ({}));
    if(!response.ok || !data.cover?.url) throw new Error(tr('canvas.saveFailed'));
    return data.cover.url;
}
async function saveCurrentCanvasPromptAsTemplate(nodeId=promptTemplateNodeId){
    const node = nodes.find(item => item.id === nodeId && item.type === 'prompt');
    const text = String(node?.text || '').trim();
    if(!text){ setStatus(tr('canvas.currentPromptEmpty')); return; }
    if(canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        const outcome = await classicPromptCommitLane().commitPrompt({
            action:'create',
            name:String(node.title || '').trim() || canvasPromptTemplateDefaultName(text),
            positive:text
        });
        const data = outcome.data;
        promptTemplateSelectedId = data.item?.id || '';
        const savedId = data.item?.id || '';
        const edit = () => openPromptTemplateModal(nodeId).then(() => {
            promptTemplateSelectedId = savedId;
            renderPromptTemplateModal();
            promptTemplatePanel?.openEdit?.(savedId);
        });
        classicPromptNotify(tr('canvas.savedToCurrentCanvas'), {tone:'success', duration:5000, actionLabel:tr('common.edit'), onAction:edit});
    } catch(error) {
        setStatus(error.message || tr('canvas.saveFailed'));
    } finally {
        canvasPromptTemplateSaving = false;
        renderPromptTemplateModal();
    }
}
...
async function saveCanvasPromptTemplateEdit(detail={}){
    const creating = Boolean(detail.creating);
    const item = creating ? null : activeCanvasPromptLibraryItems().find(template => template.id === detail.templateId);
    if(!creating && !item) return;
    const draft = detail.draft || {};
    const name = String(draft.name || '').trim();
    const positive = String(draft.positive || '').trim();
    if(!name || !positive || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        const cover = draft.coverFile ? await uploadClassicPromptCover(draft.coverFile) : String(draft.cover || '');
        let data;
        if(activePromptLibraryId === 'canvas'){
            ...
            const category = activeCanvasPromptTemplateGroups().find(group => group.id === draft.category);
            if(!category) throw new Error(tr('smart.createGroupFirst'));
            const sourceId = item?.source_id || item?.sourceId || item?.id || '';
            const response = await fetch(creating ? '/api/prompt-libraries/items' : `/api/prompt-libraries/items/${encodeURIComponent(sourceId)}`, {
                method:creating ? 'POST' : 'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({
                    library_id:item?.library_id || category.library_id || '',
                    name, category:category.category_id || draft.category, positive,
                    negative:item?.negative || '', cover
                })
            });
        ...
        if(activePromptLibraryId === 'canvas'){
            ...
            applyClassicCommonPromptResponse(data);
            const commonItem = classicCommonPromptLibrary().items.find(record => record.source_id === data.item?.id);
            promptTemplateSelectedId = commonItem?.id || item?.id || '';
        }
        promptTemplatePanel?.closeEditor?.();
        renderPromptTemplateModal();
        classicPromptNotify(tr(creating ? 'canvas.promptCreated' : 'canvas.promptSaved'), {tone:'success'});
    } catch(error) {
        setStatus(error.message || tr('canvas.saveFailed'));
    } finally {
        canvasPromptTemplateSaving = false;
...
async function deleteCanvasPromptTemplate(templateId=promptTemplateSelectedId){
    const item = activeCanvasPromptLibraryItems().find(template => template.id === templateId);
    if(!item || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        let data;
        if(activePromptLibraryId === 'canvas'){
            ...
            const response = await fetch(`/api/prompt-libraries/items/${encodeURIComponent(item.source_id || item.sourceId || item.id)}?${query}`, {method:'DELETE'});
            data = await response.json().catch(() => ({}));
            if(!response.ok) throw new Error(data.detail?.message || data.detail || tr('canvas.deleteFailed'));
        }
        if(activePromptLibraryId !== 'canvas') applyClassicCommonPromptResponse(data);
        promptTemplateSelectedId = '';
        promptTemplatePanel?.closeEditor?.();
        renderPromptTemplateModal();
    } catch(error) {
        setStatus(error.message || tr('canvas.deleteFailed'));
    } finally {
        canvasPromptTemplateSaving = false;
        renderPromptTemplateModal();
    }
}
async function copyClassicPromptTemplateToCanvas(templateId){
    const item = activeCanvasPromptLibraryItems().find(template => template.id === templateId);
    if(!item || activePromptLibraryId !== 'common' || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        await classicPromptCommitLane().commitPrompt({
            action:'copy',
            sourceItemId:item.source_id || item.sourceId || item.id,
            libraryId:item.library_id || '',
        });
        classicPromptNotify(tr('canvas.copiedToCurrentCanvas'), {tone:'success'});
    } catch(error) {
        setStatus(error.message || tr('canvas.copyPromptFailed'));
    } finally {
        canvasPromptTemplateSaving = false;
        renderPromptTemplateModal();
    }
}
async function promoteClassicPromptTemplate(detail={}){
    const item = activeCanvasPromptLibraryItems().find(template => template.id === detail.templateId);
    if(!item || activePromptLibraryId !== 'canvas' || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        const outcome = await classicPromptCommitLane().commitPrompt({
        ...
        promptTemplateSelectedId = '';
        renderPromptTemplateModal();
        classicPromptNotify(tr('canvas.setAsCommon'), {tone:'success'});
    } catch(error) {
        setStatus(error.message || tr('canvas.saveFailed'));
    } finally {
        canvasPromptTemplateSaving = false;
...
async function moveClassicPromptTemplateToCategory(detail={}){
    const item = activeCanvasPromptLibraryItems().find(template => template.id === detail.templateId);
    const category = activeCanvasPromptTemplateGroups().find(group => group.id === detail.categoryId);
    if(!item || !category || activePromptLibraryId !== 'common' || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        const response = await fetch(`/api/prompt-libraries/items/${encodeURIComponent(item.source_id || item.sourceId || item.id)}`, {
            method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({
                library_id:item.library_id || category.library_id || '', name:item.name || '',
                category:category.category_id || category.id, positive:item.positive || '',
                negative:item.negative || '', cover:item.cover || ''
            })
        });
        const data = await response.json().catch(() => ({}));
        if(!response.ok) throw new Error(data.detail?.message || data.detail || tr('canvas.saveFailed'));
...
function classicCommonWriteLibraryId(){
    const common = classicCommonPromptLibrary();
    return common.categories?.[0]?.library_id || common.items?.[0]?.library_id || '';
}
async function createCanvasPromptTemplateGroup(detail={}){
    const name = String(detail.name || '').trim();
    if(!name || activePromptLibraryId !== 'common' || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        const response = await fetch('/api/prompt-libraries/categories', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:name.slice(0, 24), library_id:classicCommonWriteLibraryId()})});
        const data = await response.json().catch(() => ({}));
        if(!response.ok) throw new Error(data.detail || tr('canvas.addGroupFailed'));
        applyClassicCommonPromptResponse(data);
        const created = activeCanvasPromptTemplateGroups().find(group => group.category_id === data.category?.id);
        promptTemplateCategory = created?.id || 'all';
        promptTemplatePanel?.closeCategoryEditor?.();
    } catch(error) {
        setStatus(error.message || tr('canvas.addGroupFailed'));
    } finally {
        canvasPromptTemplateSaving = false;
        renderPromptTemplateModal();
    }
}
async function renameCanvasPromptTemplateGroup(detail={}){
    const group = activeCanvasPromptTemplateGroups().find(item => item.id === detail.categoryId);
    const name = String(detail.name || '').trim();
    if(!group || !name || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        const response = await fetch(`/api/prompt-libraries/categories/${encodeURIComponent(group.category_id || group.id)}`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:name.slice(0, 24), library_id:group.library_id || ''})});
        const data = await response.json().catch(() => ({}));
        if(!response.ok) throw new Error(data.detail || tr('canvas.renameFailed'));
        applyClassicCommonPromptResponse(data);
        promptTemplatePanel?.closeCategoryEditor?.();
    } catch(error) {
        setStatus(error.message || tr('canvas.renameFailed'));
    } finally {
        canvasPromptTemplateSaving = false;
        renderPromptTemplateModal();
    }
...
async function deleteCanvasPromptTemplateGroup(groupId){
    const group = activeCanvasPromptTemplateGroups().find(item => item.id === groupId);
    if(!group || canvasPromptTemplateSaving) return;
    canvasPromptTemplateSaving = true;
    renderPromptTemplateModal();
    try {
        const query = new URLSearchParams({library_id:group.library_id || ''});
        const response = await fetch(`/api/prompt-libraries/categories/${encodeURIComponent(group.category_id || group.id)}?${query}`, {method:'DELETE'});
        const data = await response.json().catch(() => ({}));
...
async function persistClassicPromptTemplateGroupOrder(groupIds=[]){
    const groups = activeCanvasPromptTemplateGroups();
    const ordered = groupIds.map(id => groups.find(group => group.id === id)).filter(Boolean);
    ...
    try {
        for(const [libraryId, ids] of byLibrary){
            const response = await fetch(`/api/prompt-libraries/${encodeURIComponent(libraryId)}/categories/order`, {method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({category_ids:ids})});
            const data = await response.json().catch(() => ({}));
            if(!response.ok) throw new Error(data.detail || tr('canvas.saveFailed'));
            applyClassicCommonPromptResponse(data);
        }
    } catch(error) {
        setStatus(error.message || tr('canvas.saveFailed'));
        await loadCanvasPromptTemplates().catch(() => {});
    } finally {
        canvasPromptTemplateSaving = false;
        renderPromptTemplateModal();
    }
...
function renderLoopBody(node){
    ...
    const loopRunHtml = loopTargetId ? (isCascadeActive(loopTargetId)
        ? `<div class="gen-run-row"><button class="gen-cascade-btn gen-cascade-stop" type="button" data-loop-cascade-stop="${loopTargetId}" ${isCascadeStopping(loopTargetId) ? 'disabled' : ''}><i data-lucide="square" class="w-4 h-4"></i><span>${tr(isCascadeStopping(loopTargetId) ? 'canvas.stopping' : 'canvas.stopRunning')}</span></button></div>`
        : `<div class="gen-run-row"><button class="gen-cascade-btn" type="button" data-loop-cascade="${loopTargetId}" title="${tr('canvas.runFromLoop')}"><i data-lucide="play-circle" class="w-4 h-4"></i><span>${trf('canvas.startNodes', {nodes:loopTargetOrder.length || 1, count:node.count, rounds:tr('canvas.loopRounds')})}</span></button></div>`)
        : '';
    wrap.innerHTML = `
        ...
        ${node.showPrompt ? `<div class="loop-prompt-panel ${hasUpstreamPrompt ? 'has-upstream' : ''}">
            ...
            ${hasUpstreamPrompt ? `<div class="loop-prompt-hint">${trf('canvas.promptsDetected', {count:promptItemCount})}</div>` : ''}
    ...
    countInput.oninput = e => {
        ...
        if(loopCascadeBtn){
            ...
            if(span) span.textContent = trf('canvas.startNodes', {nodes:loopTargetOrder.length || 1, count:node.count, rounds:tr('canvas.loopRounds')});
        }
        if(loopTargetId){
            ...
            if(targetCascadeBtn){
                ...
                if(span){
                    ...
                    span.textContent = trf('canvas.runNodes', {count: targetOrder.length, suffix: ` × ${node.count} ${tr('canvas.loopRounds')}`});
                }
            }
        }
        scheduleSave();
...
function renderLLMBody(node){
    ...
    const selectedEntry = catalogEntry('text', node.llmProvider || '', node.model || node.llmMsModel || '');
    node.llmProvider = selectedEntry?.provider_id || resolveChatProviderId(node.llmProvider || 'comfly');
    node.model = selectedEntry?.model || node.model;
    ...
    const mediaBadgeText = [
        imgs.length ? trf('canvas.connectedImages', {count:imgs.length}) : '',
        videos.length ? trf('canvas.connectedVideos', {count:videos.length}) : ''
    ].filter(Boolean).join(' · ');
    const imgBadge = mediaBadgeText ? `<div style="display:flex;align-items:center;gap:var(--ui-space-1);padding:var(--ui-space-1) var(--ui-space-2);border-radius:var(--ui-radius-s);background:var(--ui-color-surface);color:var(--ui-color-text-primary);font-size:var(--ui-font-size-1);font-weight:var(--ui-font-weight-bold);width:fit-content;line-height:var(--ui-line-height-compact)"><i data-lucide="${videos.length && !imgs.length ? 'video' : 'image'}" class="w-3 h-3"></i>${trf('canvas.connectedMedia', {media:mediaBadgeText})}</div>` : '';
    node.showSystem = Boolean(node.showSystem);
    wrap.innerHTML = `
        <div class="llm-row">
            ${canvasModelVendorIconMarkup('text', llmProv, node.model)}
            <select class="select-lite llm-model" style="flex:1">${modelOpts}</select>
            <span class="model-platform-tag">${escapeHtml(catalogPlatformName('text', llmProv, node.model))}</span>
    ...
    [modelSelect].forEach(input => {
        input.onmousedown = e => e.stopPropagation();
        input.onclick = e => e.stopPropagation();
    });
    modelSelect.onchange = e => {
        e.stopPropagation();
        const entry = catalogEntryById('text', e.target.value);
        if(!entry) return;
        node.llmProvider = entry.provider_id;
        node.model = entry.model;
        if(node.llmProvider === 'modelscope') node.llmMsModel = entry.model;
        syncCanvasModelVendorIcon(wrap, entry);
        wrap.querySelector('.model-platform-tag').textContent = entry.provider_name || entry.provider_id;
        scheduleSave();
    };
...
function renderLLMNodePane(container, node){
    ...
    const inputPlaceholder = tr('canvas.llmInputPlaceholder');
    container.innerHTML = `
        <div class="llm-pane-label">Input${isReadonly ? ` <span style="font-size:var(--ui-font-size-1);opacity:.5;font-weight:var(--ui-font-weight-medium);text-transform:none;letter-spacing:var(--ui-letter-spacing-normal)">(${tr('canvas.fromConnection')})</span>` : ''}</div>
        ...
        <div class="llm-output-wrap" style="height:${outputHeight}px; flex:0 0 ${outputHeight}px;">
            <button class="llm-copy-btn llm-output-copy" type="button" title="${tr('canvas.copy')}"><i data-lucide="copy" class="w-3.5 h-3.5"></i></button>
            <div class="llm-output llm-result-output">${escapeHtml(node.outputText || tr('canvas.llmOutputEmpty'))}</div>
        </div>
        <div class="gen-run-row mt-2">
            <button class="llm-run ${node.running ? 'running' : ''}" ${node.running ? 'disabled' : ''}><i data-lucide="play" class="w-4 h-4"></i>${node.running ? tr('canvas.running') : tr('canvas.runLlm')}</button>
            ${cascadeBtnHtml(node)}
        </div>
        ${retryBarHtml(node)}
    `;
...
function renderLLMChatPane(container, node){
    ...
    container.innerHTML = `
        <div class="llm-chat-log">${messages.length ? messages.map((msg, mi) => `<div class="llm-bubble ${msg.role === 'user' ? 'user' : 'assistant'}" data-msg-idx="${mi}">${escapeHtml(msg.content || '')}${msg.role === 'assistant' ? `<button class="llm-bubble-copy" type="button" title="${tr('canvas.copy')}"><i data-lucide="copy" style="width:11px;height:11px;display:inline-block;vertical-align:middle"></i></button>` : ''}</div>`).join('') : `<div class="text-[11px] text-gray-300">${tr('canvas.startChat')}</div>`}</div>
        <textarea class="llm-chat-input mt-2" rows="2" placeholder="${tr('canvas.chatInput')}">${escapeHtml(node.chatInput || '')}</textarea>
        <button class="llm-run mt-2" ${node.running ? 'disabled' : ''}><i data-lucide="send" class="w-4 h-4"></i>${node.running ? tr('canvas.sending') : tr('canvas.send')}</button>
    `;
...
function renderGeneratorBody(node){
    ...
    wrap.innerHTML = `
        ...
        <div class="gen-settings">
            ...
            <div class="gen-settings-row custom-size-row" style="display:none">
                ...
                <button class="secondary-btn fit-size-btn" type="button" style="height:var(--ui-control-height-s);align-self:flex-end;padding:var(--ui-space-0) var(--ui-space-2);font-size:var(--ui-font-size-2)">${tr('canvas.fitImageSize')}</button>
            </div>
        </div>
        <div class="gen-run-row">
    ...
    modelSelect.onchange = e => {
        e.stopPropagation();
        const entry = catalogEntryById('image', e.target.value);
        if(!entry) return;
        node.apiProvider = entry.provider_id;
        node.model = entry.model;
        syncCanvasModelVendorIcon(wrap.querySelector('.gen-settings-row'), entry);
        wrap.querySelector('.model-platform-tag').textContent = entry.provider_name || entry.provider_id;
...
function renderVideoBody(node){
    ...
    wrap.innerHTML = `
        ...
        <div class="video-input-head">
            ...
            <div class="video-input-actions">
                <button type="button" class="tool-btn" data-video-manual-url title="${tr('canvas.manualVideoUrl')}"><i data-lucide="link" class="w-4 h-4"></i><span>${tr('canvas.inputUrl')}</span></button>
                <button type="button" class="tool-btn" data-video-temp-sh ${node.tempShUploading ? 'disabled' : ''} title="${tr('canvas.uploadCurrentVideo')}"><i data-lucide="upload-cloud" class="w-4 h-4"></i><span>${tr(node.tempShUploading ? 'canvas.uploading' : 'canvas.cloudUpload')}</span></button>
            </div>
        </div>
        <div class="input-list video-img-list"></div>
        <div class="gen-settings">
            <div class="gen-settings-row">
                ${canvasModelVendorIconMarkup('video', node.apiProvider, node.model)}
                <select class="select-lite video-model" style="flex:1">${videoModelOptions(node.model, node.apiProvider)}</select>
                <span class="model-platform-tag">${escapeHtml(catalogPlatformName('video', node.apiProvider, node.model))}</span>
            </div>
            <div class="gen-settings-row">
                <label class="field" style="flex:1">
    ...
    [modelSelect, durationSelect, aspectSelect, resolutionSelect].forEach(input => {
        input.onmousedown = e => e.stopPropagation();
        input.onclick = e => e.stopPropagation();
    });
    modelSelect.onchange = e => {
        e.stopPropagation();
        const entry = catalogEntryById('video', e.target.value);
        if(!entry) return;
        node.apiProvider = entry.provider_id;
        node.model = entry.model;
        syncCanvasModelVendorIcon(wrap.querySelector('.gen-settings-row'), entry);
    ...
    wrap.querySelectorAll('[data-video-temp-sh]').forEach(btn => {
        ...
        btn.onclick = async e => {
            ...
            try {
                ...
                showErrorModal(err.message || tr('canvas.cloudUploadFailed'), tr('canvas.cloudUpload'));
            }
        };
    });
    wrap.querySelectorAll('[data-video-manual-url]').forEach(btn => {
        ...
        btn.onclick = async e => {
            ...
            try {
                ...
                showErrorModal(err.message || tr('canvas.setVideoUrlFailed'), tr('canvas.inputUrl'));
            }
        };
    });
...
function renderVideoImageInputs(list, node, imageInputs){
    ...
    imageInputs.forEach((src, i) => {
        ...
        const typeLabel = trf(kind === 'audio' ? 'canvas.audioNumber' : kind === 'video' ? 'canvas.videoNumber' : 'canvas.imageNumber', {number:i + 1});
        item.innerHTML = `
...
function runningHubEntryLabel(entry, kind){
    ...
    return entry?.title || entry?.name || trf(kind === 'workflow' ? 'canvas.rhWorkflowFallback' : 'canvas.rhAppFallback', {id:id.slice(-6)});
}
...
function rhEntryOptions(selected){
    ...
    if(!models.length && !apps.length && !workflows.length) return `<option value="">${tr('canvas.rhNoConfigHint')}</option>`;
    ...
    ` : '';
    return `${group('model', models, tr('canvas.rhModelApi'))}${group('app', apps, tr('canvas.rhAiApp'))}${group('workflow', workflows, tr('canvas.workflow'))}`;
}
function rhPaymentOptions(node){
    const provider = runningHubProvider();
    const selected = node.rhPayment === 'wallet' ? 'wallet' : 'free';
    return `
        <option value="free" ${selected === 'free' ? 'selected' : ''}>${tr('canvas.rhCoinKey')}${provider?.has_key ? '' : tr('canvas.keyUnconfigured')}</option>
        <option value="wallet" ${selected === 'wallet' ? 'selected' : ''}>${tr('canvas.rhWalletKey')}${provider?.has_wallet_key ? '' : tr('canvas.keyUnconfigured')}</option>
    `;
}
...
async function rhBuildWorkflowRequestExtras(node, media, nodeInfoList){
    ...
    for(const field of fields){
        ...
        if(field.required === true && !hasInput){
            throw new Error(trf('canvas.rhRequiredImageMissing', {name:rhRequiredLabel(field)}));
        }
        if(field.required !== true && !hasInput){
    ...
    wrap.innerHTML = `
        <div class="rh-top">
            <label class="field rh-webapp-field">
                <div class="setting-title">${tr('canvas.rhConfig')}</div>
            ...
            <label class="field rh-machine-field" style="${mode === 'model' ? 'display:none' : ''}">
                <div class="setting-title">${tr('canvas.vram')}</div>
        ...
        <div class="rh-param-head">
            <span>${mode === 'model' ? tr('canvas.modelApiParams') : mode === 'workflow' ? tr('canvas.rhWorkflowParams') : tr('canvas.rhParams')}</span>
...
function renderRhPromptFields(container, node, fields){
    ...
    container.innerHTML = prompts.map(field => {
        const key = rhParamKey(field.nodeId, field.fieldName);
        const label = field.label || field.fieldName || tr('canvas.prompt');
    ...
    if(rhRandomEnabled(field)){
        ...
        return `<div class="gen-settings-row rh-param-row ${wide ? 'wide' : ''}">
            <div class="comfy-random-field">
                ...
                <button class="tool-btn comfy-random-btn ${active ? 'active' : ''}" type="button" data-rh-random="${escapeAttr(key)}" title="${tr(active ? 'canvas.randomOn' : 'canvas.randomOff')}"><i data-lucide="dice-5" class="w-4 h-4"></i></button>
            </div>
        </div>`;
    }
...
async function runRhNode(nodeId, opts={}){
    ...
    if(!selectedEntry){
        alert(tr(mode === 'workflow' ? 'canvas.rhAddWorkflowFirst' : 'canvas.rhAddAppFirst'));
        return;
    }
    if(mode === 'workflow') await ensureRunningHubWorkflowConfigForNode(node);
    if(!rhActiveFields(node).length){
        alert(tr(mode === 'workflow' ? 'canvas.rhSaveWorkflowParams' : 'canvas.rhSaveAppParams'));
        return;
    }
    const media = rhMediaSources(node);
...
function renderComfyCustomField(node, f){
    ...
    if(f.type === 'dropdown'){
        ...
        return `<div class="gen-settings-row">
            <label class="field" style="flex:1"><div class="setting-title">${label}</div><select class="select-lite" data-comfy-param="${escapeHtml(f.id)}" data-comfy-type="dropdown" style="width:100%">${opts || `<option value="">${tr('canvas.noOption')}</option>`}</select></label>
        </div>`;
    }
    if(f.type === 'textarea'){
        return `<div class="gen-settings-row">
            <label class="field" style="flex:1"><div class="setting-title">${label}</div><textarea class="setting-input" data-comfy-param="${escapeHtml(f.id)}" data-comfy-type="textarea" style="height:66px;padding-top:var(--ui-space-2);resize:vertical">${escapeHtml(value)}</textarea></label>
        </div>`;
    }
    ...
    if(comfyRandomEnabled(f)){
        ...
        return `<div class="gen-settings-row">
            <div class="comfy-random-field">
                ...
                <button class="tool-btn comfy-random-btn ${active ? 'active' : ''}" type="button" data-comfy-random="${escapeHtml(f.id)}" title="${tr(active ? 'canvas.randomOn' : 'canvas.randomOff')}" aria-label="${tr(active ? 'canvas.randomOn' : 'canvas.randomOff')}"><i data-lucide="dice-5" class="w-4 h-4"></i></button>
            </div>
        </div>`;
    }
    return `<div class="gen-settings-row">
    ...
    return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        if(n.type === 'output' && (n.images||[]).length){
            ...
            if(found){
                ...
                return {id:n.id, type:'outputImage', label:tr('canvas.upstreamOutput'), preview:last, refs:[{url:last, name:'output.png', kind, nodeId:n.id, outputIndex:found.index}], prompt:''};
            }
        }
        if(CANVAS_MEDIA_OUTPUT_TYPES.includes(n.type)){
            ...
            if(refs.length){
                return refs.map((ref, i) => ({
                    ...
                    type:'generatedImage',
                    label:trf('canvas.upstreamGenerated', {number: i + 1}),
                    preview:ref.url,
                    refs:[ref],
                    prompt:''
                }));
        ...
        if(n.type === 'prompt') return {id:n.id, type:'prompt', label:(n.text || tr('canvas.prompt')).slice(0, 32), refs:[], prompt:n.text || ''};
        if(n.type === 'loop') {
        ...
        if(n.type === 'promptGroup') {
            ...
            return {id:n.id, type:'promptGroup', label:trf('canvas.promptGroupLabel', {count: prompts.length}), refs:[], prompt:prompts.join('\n\n')};
        }
...
async function uploadCanvasUrlToComfy(url){
    const blob = await fetch(url).then(r => {
        if(!r.ok) throw new Error(tr('canvas.imageReadFailed'));
        return r.blob();
    });
    ...
    const data = await fetch('/api/upload', {method:'POST', body:form}).then(async r => {
        if(!r.ok) throw new Error(await responseErrorMessage(r, tr('canvas.comfyImageUploadFailed')));
        return r.json();
    });
...
async function comfyNameForRef(ref){
    ...
    if(!ref.url) throw new Error(tr('canvas.missingInputImage'));
    return uploadCanvasUrlToComfy(ref.url);
}
async function runComfyUpscale(imageUrl, resolution, options={}){
    if(!imageUrl) throw new Error(actionFailed('studio.superResolution', tr('canvas.missingInputImage')));
...
function cascadeAbortError(message=tr('canvas.cascadeStopped')){
...
function cascadeStopMessage(reason=''){
    ...
    return tr('canvas.cascadeStopped');
}
function cascadeBackendRestartMessage(){
    return tr('canvas.cascadeBackendRestarted');
}
...
async function runComfyNode(nodeId, opts={}){
    ...
    if(mode === 'custom' && videoRefsOnly(allRefs).length < customVideoFields.length){ alert(tr('canvas.comfyNeedVideoInputs')); return; }
    if(mode === 'custom' && audioRefsOnly(allRefs).length < customAudioFields.length){ alert(tr('canvas.comfyNeedAudioInputs')); return; }
...
async function callCanvasLLM(node, message, messages=[], options={}){
    ...
    const result = await cascadeFetch('/api/canvas-llm', {
        ...
        if(!r.ok){
            throw new Error(await responseErrorMessage(r, tr('canvas.llmRunFailed')));
        }
        return r.json();
    });
    return result.text || '';
...
async function runLLMNode(nodeId, opts={}){
    ...
    if(!input){
        if(opts.cascade) throw new Error(tr('canvas.llmNeedPrompt'));
    ...
    try {
        ...
        refreshNodes([node.id]);
        if(opts.cascade) throw err;
        alert(err.message || tr('canvas.llmRunFailed'));
    }
}
...
function cascadeBtnHtml(node){
    ...
    if(isCascadeActive(node.id)){
        ...
        return `<button class="gen-cascade-btn gen-cascade-stop" type="button" data-cascade-stop="${node.id}" ${stopping ? 'disabled' : ''}><i data-lucide="square" class="w-4 h-4"></i><span>${tr(stopping ? 'canvas.stopping' : 'canvas.stopRunning')}</span></button>`;
    }
    return `<button class="gen-cascade-btn" type="button" data-cascade="${node.id}" title="${tr('canvas.runWorkflow')}"><i data-lucide="play-circle" class="w-4 h-4"></i><span>${trf('canvas.runNodes', {count:order.length, suffix})}</span></button>`;
}
function retryBarHtml(node){
    ...
    return `<div class="node-retry-bar" data-retry-bar>
        ...
        <button class="node-retry-btn" type="button" data-retry="${node.id}">${tr('canvas.retry')}</button>
        <button class="node-stop-btn" type="button" data-stop="${node.id}">${tr('canvas.stop')}</button>
    </div>`;
}
...
async function runNodeCascade(nodeId){
    ...
    if(!target) return;
    if(target.running){ alert(tr('canvas.nodeRunning')); return; }
    const order = computeCascadeOrder(nodeId);
    if(!order.length){ alert(tr('canvas.noRunnableNodes')); return; }
...
function renderCanvasLog(){
    ...
    list.innerHTML = logs.length ? logs.map(log => {
        ...
        const subParts = [
            date,
            trf('canvas.outputsCount', {count: (log.outputs || []).length}),
            idText ? `ID ${idText}` : '',
            backendText,
        ].filter(Boolean);
...
async function queryRecoverPendingOutput(pendingId){
    ...
    if(!taskId){
        showErrorModal(tr('canvas.noTaskIdQuery'), tr('canvas.apiFailed'));
        return;
    }
    ...
    try {
        ...
        if(!res.ok) throw new Error(await responseErrorMessage(res, tr('canvas.queryFailed')));
        const data = await res.json();
        ...
        if(data.status === 'failed'){
            ...
            pending.error = data.message || tr('canvas.taskStillRunning');
            setStatus(pending.error);
        }
    } catch(err) {
        pending.error = err.message || tr('canvas.queryFailed');
        showErrorModal(pending.error, tr('canvas.apiFailed'));
    } finally {
...
function renderOutputMedia(item, useGridLayout=false){
    ...
    if(kind === 'video'){
        return `<div class="output-img-wrap" data-output-url="${safe}"${gridStyle}>${canvasVideoPreviewHtml(url, useGridLayout ? 512 : 768, 'alt="video output" data-video-fallback-attrs="controls data-output-video-fallback=&quot;1&quot;"')}${timePill}<button class="canvas-video-play output-video-play" type="button" title="${tr('canvas.play')}"><i data-lucide="play"></i></button><div class="output-video-badge"><i data-lucide="play" class="w-3 h-3"></i>VIDEO</div><button class="output-del" title="${tr('common.delete')}">×</button></div>`;
    }
    if(kind === 'audio'){
...
async function downloadUrl(url, filename){
    const res = await fetch(url);
    if(!res.ok) throw new Error(tr('canvas.downloadFailed'));
    const blob = await res.blob();
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = filename;
...
promptTemplatePanel?.addEventListener('ic-library-change', event => {
    activePromptLibraryId = event.detail?.libraryId === 'common' ? 'common' : 'canvas';
    promptTemplateCategory = 'all';
    promptTemplateSelectedId = '';
    promptTemplatePanel.query = '';
    promptTemplatePanel.closeEditor?.();
    promptTemplatePanel.manageMode = false;
    renderPromptTemplateModal();
    if(event.detail?.action === 'create-category' && activePromptLibraryId === 'common') promptTemplatePanel.openCategoryEditor?.('create');
});
...
promptTemplatePanel?.addEventListener('ic-template-select', event => {
    ...
    // Classic Canvas keeps its existing one-click application behavior. The
    // Prompt Node stores the actual text, so later template edits never rewrite it.
    applyPromptTemplateToPromptNode('positive');
});
promptTemplatePanel?.addEventListener('ic-template-create', event => saveCanvasPromptTemplateEdit({...event.detail, creating:true}));
promptTemplatePanel?.addEventListener('ic-template-edit', event => saveCanvasPromptTemplateEdit({...event.detail, creating:false}));
promptTemplatePanel?.addEventListener('ic-template-delete', event => deleteCanvasPromptTemplate(event.detail?.templateId || ''));
promptTemplatePanel?.addEventListener('ic-template-copy', event => copyClassicPromptTemplateToCanvas(event.detail?.templateId || ''));
promptTemplatePanel?.addEventListener('ic-template-promote', event => promoteClassicPromptTemplate(event.detail || {}));
promptTemplatePanel?.addEventListener('ic-template-move', event => moveClassicPromptTemplateToCategory(event.detail || {}));
promptTemplatePanel?.addEventListener('ic-template-reorder', event => {
    if(event.detail?.scope === 'categories') persistClassicPromptTemplateGroupOrder(event.detail?.categoryIds || []);
});
promptTemplatePanel?.addEventListener('ic-category-create', event => createCanvasPromptTemplateGroup(event.detail || {}));
promptTemplatePanel?.addEventListener('ic-category-edit', event => renameCanvasPromptTemplateGroup(event.detail || {}));
promptTemplatePanel?.addEventListener('ic-category-delete', event => deleteCanvasPromptTemplateGroup(event.detail?.categoryId || ''));
promptTemplatePanel?.addEventListener('ic-close', async event => {
    const nodeId = promptTemplateNodeId;
    await closePromptTemplateModal(event);
    requestAnimationFrame(() => document.querySelector(`[data-prompt-template-open][data-prompt-template-node-id="${CSS.escape(nodeId)}"]`)?.focus?.({preventScroll:true}));
});
promptTemplateDialog?.addEventListener('ic-hide', closePromptTemplateModal);
...
workflowImportDropZone?.addEventListener('drop', event => {
    ...
    if(file) importWorkflowFile(file);
    else setStatus(tr('canvas.workflowFileRequired'));
});
...
function openOutputLightbox(url, out){
    ...
    if(videoMode){
        ...
        outputDownloadBtn.onclick = e => {
            e.stopPropagation();
            downloadUrl(url, outputDownloadName(url)).catch(err => alert(err.message || tr('canvas.downloadFailed')));
        };
...
function updateWorkflowTransferMeta(){
    ...
    if(workflowExportMeta) workflowExportMeta.textContent = nodeCount ? trf('canvas.selectedNodesMeta', {nodes:nodeCount, connections:connCount}) : tr('canvas.selectNodesToExport');
    if(workflowTransferSub) workflowTransferSub.textContent = tr(nodeCount ? 'canvas.exportCurrentOrImport' : 'canvas.selectBeforeExport');
}
async function exportSelectedWorkflow(includeResources=false){
    if(!canvas) return;
    const payload = selectedWorkflowPayload();
    if(!payload.nodes.length){
        if(workflowExportMeta) workflowExportMeta.textContent = tr('canvas.selectNodesToExport');
        if(workflowTransferSub) workflowTransferSub.textContent = tr('canvas.selectBeforeExport');
        setStatus(tr('canvas.selectNodesToExport'));
        return;
    }
    try {
        if(!includeResources){
            ...
            setStatus(tr('canvas.exportedJson'));
            return;
        }
        ...
        if(!res.ok) throw new Error(await responseErrorMessage(res, tr('canvas.exportWorkflowFailed')));
        const blob = await res.blob();
        downloadBlob(blob, filename);
        setStatus(tr('canvas.exportedPackage'));
    } catch(err) {
        showErrorModal(err.message || tr('canvas.exportWorkflowFailed'), tr('canvas.exportWorkflow'));
    }
}
...
function insertWorkflowIntoCanvas(imported){
    ...
    render();
    scheduleSave();
    setStatus(trf('canvas.importedNodes', {count:newNodes.length}));
}
async function importWorkflowFile(file){
    ...
    try {
        ...
        if(!res.ok) throw new Error(await responseErrorMessage(res, tr('canvas.importWorkflowFailed')));
        const data = await res.json();
        insertWorkflowIntoCanvas(normalizeImportedWorkflow(data));
        closeWorkflowTransferModal();
    } catch(err) {
        showErrorModal(err.message || tr('canvas.importWorkflowFailed'), tr('canvas.importWorkflow'));
    }
}
...
function isZoomPreviewIgnoredTarget(target){
    return !!target?.closest?.('#createMenu, #linkCreateMenu, #nodeInputMenu, #nodeOutputMenu, #imageNodeMenu, .minimap, #workflowTransferModal, #logModal, #promptTemplateModal, #imageEditModal, #outputLightbox');
}
...
window.addEventListener('keydown', e => {
    ...
    if(e.key === 'Escape' && promptTemplateDialog?.open) { closePromptTemplateModal(); return; }
