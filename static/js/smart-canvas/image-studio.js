...
function layerDecompositionEditorNode(){
    return nodes.find(node => (
        node.id === layerDecompositionEditNodeId
...
function renderLayerDecompositionEditor(){
    ...
    if(!node || !editor || !composite || !list) return false;
    ...
    list.innerHTML = items.slice().reverse().map(item => {
        const media = imageForDisplay(item.media);
        const name = item.role === 'base'
            ? tr('smart.layerDecompositionBase')
            : media.name || tr('smart.layerDecomposition');
        ...
        return `<div class="layer-decomposition-editor-layer${item.hidden ? ' is-hidden' : ''}" role="listitem" data-layer-item-id="${escapeAttr(item.id)}">
...
function applyLayerDecompositionEditorAction(itemId, action){
    if(!itemId || !['visibility','delete'].includes(action)) return;
    const node = layerDecompositionEditorNode();
    ...
    if(action === 'visibility'){
        ...
        node.layerDecompositionItems.splice(index,1);
    }
...
function openLayerDecompositionEditor({nodeId}={}){
    const node = nodes.find(candidate => (
        candidate.id === nodeId
        && candidate.type === 'smart-layer-decomposition'
    ));
    ...
    document.getElementById('imageEditStage')?.classList.add('preview-mode');
    renderLayerDecompositionEditor();
    if(!dialogWasOpen){
