...
const id = 'local-generation-browser';
let released=false; const waiters=[]; const staged=[];
const gate=()=>released?Promise.resolve():new Promise(resolve=>waiters.push(resolve));
const media = n => ({url:`/fixture/source.svg?${n}`,name:`Image ${n}`,kind:'image',inputInstanceId:`image-${n}`});
const target = {id:'generation',type:'smart-image',title:'Generation',referenceGenerationKind:'image',x:500,y:150,w:280,h:220,images:[],promptDraftHtml:'Composer',promptDraftText:'Composer',manualInputRefs:[],localTextRefs:[],settings:{engine:'api',apiKind:'image',provider_id:'apimart',model:'seedream-5-0-pro',ratio:'1:1',resolution:'1K',batch:1}};
let canvas = {id,title:'Local submission acceptance',project:'default',revision:1,nodes:[
...
const server=http.createServer(async(req,res)=>{
 ...
 if(url.pathname==='/fixture/state')return send({canvas,mutations,submissions,staged,released});
 if(url.pathname==='/fixture/release'){released=true;waiters.splice(0).forEach(resolve=>resolve());return send({released});}
 if(url.pathname==='/api/local-generation-submissions'){
  if(req.method!=='POST')return send({enabled:true,workspace_id:'fixture-workspace',actor_id:'fixture-admin',submissions:staged.filter(r=>r.status!=='accepted')});
  let raw='';for await(const part of req)raw+=part;
  const command=JSON.parse(raw);const old=staged.find(r=>r.operation_id===command.operation_id&&r.request_index===command.request_index);
  if(old)return send(old);
  const record={id:command.operation_id+'-'+command.request_index,canvas_id:id,operation_id:command.operation_id,request_index:command.request_index,target_ids:command.target_ids,endpoint:command.endpoint,status:'syncing',result:{},error:''};
  staged.push(record);submissions.push(command);send(record);
  gate().then(async()=>{
   for(const operation of command.checkpoints) await fetch('http://127.0.0.1:'+server.address().port+'/fixture/mutation',{method:'POST',body:JSON.stringify({operation})});
   record.status='accepted';record.result={task_id:record.id,status:'queued'};
  });return;
 }
 if(url.pathname.startsWith('/api/local-generation-submissions/'))return send(staged.find(r=>r.id===url.pathname.split('/')[3])||{});
 if(url.pathname.startsWith('/api/canvas-image-tasks/')||url.pathname.startsWith('/api/canvas-video-tasks/'))return send({task:{id:url.pathname.split('/').at(-1),status:'running',progress:25}});
 if(url.pathname==='/fixture/mutation') {
  let raw='';for await(const part of req)raw+=part;
  const operation=JSON.parse(raw).operation; await gate();
 ...
 fs.readFile(file,(error,body)=>{
  ...
  if(url.pathname==='/static/smart-canvas.html')body=Buffer.from(body.toString().replace('<head>','<head><script src="/fixture/init.js"></script><script src="/tests/local_generation_browser_checks.js" defer></script>'));
