...
(async () => {
  ...
  try {
    ...
    assert.ok(await page.locator('[data-choice-kind="resolution"]').count() >= 5);
    ...
    assert.deepEqual(appliedGenerate.resolutions, ['1K']);
    assert.deepEqual(appliedGenerate.aspect_ratios, ['1:1']);
