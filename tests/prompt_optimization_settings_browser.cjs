...
(async()=>{
 ...
 try {
   ...
   await page.locator('#saveOptimizationSettings').getByRole('button').click();
   await page.waitForFunction(()=>!document.querySelector('#settingsForm').inert);
   assert.equal(config.repair.hand,'Repair the hand only.');assert.equal(config.repair.suffix,'Keep the style.');
   await page.reload();await page.waitForFunction(()=>!document.querySelector('#settingsForm').inert);
   await page.evaluate(()=>{const picker=document.querySelector('#optimizationMedia');picker.value='repair';picker.dispatchEvent(new Event('change',{bubbles:true}));});
   assert.equal(await input.inputValue(),'Repair the hand only.');assert.equal(await suffix.inputValue(),'Keep the style.');
