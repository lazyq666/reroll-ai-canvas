...
class ApiSettingsStandardContractTests(unittest.TestCase):
    ...
    def test_provider_card_key_action_and_plain_toolbars_follow_visual_review(self):
        ...
        self.assertIn("if(clearSavedKeyBtn) clearSavedKeyBtn.hidden = !item.has_key;", self.script)
        self.assertIn("if(btn){ btn.loading = true; btn.disabled = true; }", self.script)
        self.assertIn("if(btn){ btn.loading = false; btn.disabled = false; }", self.script)
    ...
    def test_feedback_components_keep_valid_public_contracts(self):
        ...
        self.assertIn("function showConnectionVerification(message, tone)", self.script)
        self.assertIn('id="connectionVerificationResult"', self.page)
        self.assertNotIn("setStatus(tr('api.testingUrl'))", self.script)
