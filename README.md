...
首次进入依次创建本机管理员、选择作品保存位置、选择已有 AI 服务并逐个连接。至少一个服务通过连接和模型检查后，点击“开始创作”进入画布列表；其他服务可稍后在 API 设置中添加。已有安装不强制重复引导。之后，其他用户可以在登录页提交设计师账号申请；管理员在“账号管理”页面审核申请，并为设计师分配可访问的 Project。访客账号不能进入 Canvas 编辑或管理通道。
