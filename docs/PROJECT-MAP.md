...
4. Designer 从 Canvas List 进入 Canvas；产品运行态统一使用 Smart Canvas 模型。
...
```mermaid
flowchart TB
    UI["浏览器产品层<br/>App Shell · Canvas List<br/>Smart Canvas<br/>工作台 · 设置与管理"]
...
└── infinite_canvas/
    ...
    ├── image_repair.py         局部修复的原图坐标校验、Alpha 羽化和合成
...
│   ├── media-naming.js         媒体真实扩展名、显示名校验、下载名与重命名稳定身份
...
- API Settings Package 的收集、加密、预览校验和原子合并属于 `api_settings_transfer.py`；`model_settings_backup.py` 收集所有配置模型的能力合同并通过 Workbench 一次恢复；可用模型管理页承接唯一备份入口与密码／预览交互，覆盖 CLI 模型选择、名称、排序、显隐及能力详情，保持旧包兼容。`main.py` 只组合入口。
...
| F05 | Smart Canvas 创作与交互 | `partial` | [Composer 提示词优化](active/2026-09-21-composer-prompt-optimize-spec.md)仅覆盖媒体 Composer，支持原文／优化结果持久切换，普通提示词节点不提供优化，并新增工作流设置下的优化设置页及 `/api/prompt-optimization-settings` 读写接口；通过本地回归，真实 Provider 与人工验收待完成；[Classic Canvas 退役边界](current/classic-canvas-retirement.md)规定产品运行态只允许 Smart Canvas；Image Studio 在宫格后提供“深度图”直接动作，关闭编辑器后在来源附近创建保持来源 Selection 的 Pending Node，并原位完成相对深度图；十种 Node 角色已统一通过公共 `ic-canvas-node` 外壳渲染；`/ui-component-library#nodes` 以十行稳定状态实例直接运行生产画布的拖动、选择、Resize、Quick Add、Connection、浮动工具栏与模型选择器，Image Node 行同时验收 Image、Empty、Video 与 Audio 媒体数据，Generation Node 行验收参考图片与视频生成，仅以临时会话隔离持久化和协作反馈；[UI 设计与交互指南](current/ui-design-guidelines.md)定义媒体 Composer 资格；[Smart Canvas 媒体命名与重命名](active/2026-09-20-smart-canvas-media-naming.md)已实现精确右键目标、复用 Dialog、真实扩展名和一致下载名；[批量运行节点](current/smart-canvas-batch-run-node.md)、[创建副本 Connection 继承规则](current/smart-canvas-duplicate-connection-inheritance.md)、[Node 自动避让](current/smart-canvas-node-auto-placement.md)、[选区整理](current/smart-canvas-selection-arrangement.md)、[连线与命中](current/smart-canvas-connection-quick-add-hit-priority.md)、[失败反馈](current/smart-canvas-generation-failure-feedback.md)、[预设处理器](current/smart-canvas-preset-ai-processors.md)已有 Current；[复制与粘贴的剪贴板优先级](active/2026-08-21-smart-canvas-clipboard-precedence.md)整体体验已确认，正式跨平台人工与真实环境 Gate 由 Issue #212 跟踪；[智能分层 Dialog 草稿与框选规格](active/2026-09-05-smart-canvas-layer-decomposition-dialog-spec.md)已实现随原图保存、双模式、预设和 bbox 编译，本地页面回归通过，真实 Provider / 触摸 / 双客户端 Gate 待验收；仍缺完整工具/容器/快捷键总规格 |
...
| F09 | Generation Run、Recovery、Output 与 Cascade | `drift` | [Generation Pipeline](current/generation-pipeline.md)、[ADR-0005](adr/0005-global-generation-publication-authority.md)；图片、视频与文字使用后台 task ID，Smart Canvas 通过画布级 active Run 接口恢复刷新时缺失的 Pending Node；新结果由[媒体命名合同](active/2026-09-20-smart-canvas-media-naming.md)按真实格式初始化短名称，恢复合并保护手动名称；包含确定性本地图片处理、进度持久化、无远端编号重启重跑与 `image-processor` Managed Media 发布，以及 APIMart Seedream 5.0 Pro 智能分层的同 task ID 恢复、Manifest、Managed Media 校验、专用 Layer Decomposition Node 交付与当前图层状态 PSD 导出；SQLite authority 下 Global History、Run lifecycle 与 Publication Receipt 同库且不接触三个 legacy JSON。偏差范围：Turso 盘点发现内存终态与持久化/画布交付确认不同步，以及保存、目标读取和界面恢复异常分支缺口；[链路盘点与待修复验收](active/2026-09-07-optional-cloud-records-onedrive-media-spec.md#7-turso-生成链路盘点与可靠性验收)列明基线证据；第 7.10 节已修复 G1–G5 及长任务查询，孤儿等待/active Runs 对账、专用入口差异和真实云端联合验收仍待完成 |
| F10 | Managed Media、Workspace Asset Library、Image Studio 与 Smart Matting | `partial` | [局部修复](active/2026-09-24-local-image-repair.md)由 `image-repair.js` / `image-repair-geometry.js` 负责圈选、裁剪和调整，复用图片任务并保留可编辑参数；`image_repair.py` 负责合成，`POST /api/canvases/{canvas_id}/image-repairs/{node_id}/render` 与 `/psd` 提供权限保护的合成与两层导出；真实 Provider 与 Photoshop 验收待完成。[宫格图片合成 GIF](active/2026-09-10-grid-gif.md)由图片节点菜单与共享 Dialog 发起，`grid-gif.js` / Worker 调用 `sprite-components.js` 识别完整角色，`sprite-ownership.js` 自动寻找横竖透明间隙并管理可编辑折线归属，输出复用媒体上传； [工作区资产库与本地引用](current/workspace-asset-library.md)、[手动媒体清理](current/workspace-media-cleanup.md)、[Smart Matting 性能与容量](current/smart-matting-performance.md)、[ADR-0004](adr/0004-workspace-asset-library-publication-boundary.md)已定义发布、引用保护和回收边界；Image Studio 与 Smart Matting 的统一生命周期仍是后续缺口 |
...
| F14 | 更新、回退、配置迁移与发布维护 | `partial` | `launcher.py`、更新路由和维护脚本；[前端资源版本](current/frontend-asset-versions.md)由 `sync_frontend_assets.py` 统一管理引用、依赖图与缓存合同，[生成器回归](../tests/test_frontend_assets.py)和[真实缓存升级回归](../tests/frontend_cache_upgrade_browser.cjs)覆盖漏同步阻断与命名保存；`frontend-update.js` 消费 `/api/app-info` 的版本与清单摘要，提示长期打开页面在画布保存确认后刷新，[更新提醒回归](../tests/frontend_update_browser.cjs)覆盖失败保护与父子页面协调；[Public readiness 发布验收](current/public-readiness.md)已为 Current：固定提交、Linux 独立检查与 main 强制规则已验证；[交付行为回归](../tests/test_readiness_delivery.py)覆盖隔离、版本竞争与规则漂移；其余发布维护总规格仍待补齐 |
...
Issue #29 / Linear LAZ-10 的[媒体命名与重命名](active/2026-09-20-smart-canvas-media-naming.md)为 Implemented：Image Node 与 Smart Group 的单个媒体可从文件名双击或精确右键入口复用同一 Dialog 重命名；提交按稳定媒体身份重新定位，保留真实扩展名并使 Badge 与下载名一致。Generation Output 在批量拆分前获得按媒体类型连续编号的短默认名，恢复合并保护用户名称；无新字段或迁移。本地单元与生产页面浏览器回归通过，等待合并后毕业。
