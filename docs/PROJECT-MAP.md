...
- API Settings Package 的收集、加密、预览校验和原子合并属于 `api_settings_transfer.py`；`model_settings_backup.py` 收集所有配置模型的能力合同并通过 Workbench 一次恢复；API 设置页复用同一备份入口，覆盖 CLI 模型选择、名称、排序、显隐及能力详情，保持旧包兼容。`main.py` 只组合入口。
