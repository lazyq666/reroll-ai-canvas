...
| F14 | 更新、回退、配置迁移与发布维护 | `partial` | `launcher.py`、更新路由和维护脚本；[前端资源版本](current/frontend-asset-versions.md)由 `sync_frontend_assets.py` 统一管理引用、依赖图与缓存合同，[生成器回归](../tests/test_frontend_assets.py)和[真实缓存升级回归](../tests/frontend_cache_upgrade_browser.cjs)覆盖漏同步阻断与命名保存；[Public readiness 发布验收](current/public-readiness.md)已为 Current：固定提交、Linux 独立检查与 main 强制规则已验证；[交付行为回归](../tests/test_readiness_delivery.py)覆盖隔离、版本竞争与规则漂移；其余发布维护总规格仍待补齐 |
