...
首次进入 Reroll 工作台时，应用从身份确认直接切换为完整界面，品牌的流体节点语言没有延续到产品体验。工作台现在在每个浏览器标签页的首次已登录进入中播放一次品牌入场：透明流体 Logo 完成聚合与分裂后，`word.svg` 通过节点扫描显现；整个锁定组合收束到真实的 `.sidebar-logo-image.sidebar-logo-wordmark`，再揭示已完成初始化的工作台。

登录成功使用整页跳转进入 `/`，因此与首次直接打开工作台共享同一实现；浏览器刷新无条件跳过动画，即使用户在首次动画尚未结束时刷新也不会重播。
...
- 文字必须来自 `static/images/word.svg`，终态比例与 `wordmark.svg` 一致。
...
完成标记只写入当前标签页的 `sessionStorage.studio_brand_entry_seen`。新的标签页、应用重新启动或从登录页首次进入仍播放。刷新通过 Navigation Timing 的 `reload` 类型在 `<head>` 阶段添加 `studio-entry-motion-skip`，避免动画 DOM 在延迟脚本执行前短暂闪现；运行时再执行同一判断并移除动画层。
...
1. 首次已登录进入工作台能看到透明流体 Logo、扫描文字、收束和 App Shell 淡入；任何浏览器刷新都不显示动画，包括动画中途刷新。
2. 文字网络资源是 `/static/images/word.svg`，终态与可见 `.sidebar-logo-image.sidebar-logo-wordmark` 的外框误差不超过 `1px`。
...
- 真实无头 Chrome：首次播放、终态几何、同标签刷新、WebM 失败、Reduced Motion、Light / Dark。
- 回归：T30 App Shell 静态合同和桌面 / 窄屏浏览器 Smoke。

实际验证：

- `node static/js/i18n/validate-i18n.js`：中英文资源结构、变量占位符和生产页面引用校验通过，共 `2793` 个 key。
...
- `python3 -m unittest tests.test_issue_211_studio_brand_entry_motion tests.test_t30_application_shell_migration`：11 项通过。
...
- `node tests/issue_211_studio_brand_entry_motion_browser_smoke.cjs`：真实无头 Chrome 通过正常播放、媒体失败、Dark Reduced Motion、播放完成后刷新和动画中途刷新；英文状态文案为 `Preparing your creative space…` 且没有宽度溢出。两类刷新均在首帧跳过动画。终态目标为 `112 × 30.281px`，动画组合为 `111.859 × 30.063px`，位置误差为 `0px`，宽高误差均小于 `0.25px`。
