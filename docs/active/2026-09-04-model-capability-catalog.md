...
2026-09-04 记录两类实验：可删除候选在固定回归通过后保留简化；必要边界使用临时探针验证，不把失败的消融写入正式代码。
