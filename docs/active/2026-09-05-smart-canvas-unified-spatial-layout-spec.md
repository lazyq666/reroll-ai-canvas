...
- **Status**：Implemented；Quick Add 拖线回归已修复；已实现首项固定的多图换行 / 换列
- **Feature ID**：F05；关联 F06 / F09 / F13
- **Owners**：产品 / UI / 交互 / 前端 / 后端 / 测试
- **Last verified**：2026-09-05；纯模块、Canvas Sync / WebSocket、隔离真实页面与双端读取验证完成；详见第 15 节
- **Applies to**：当前本地实现；未提交或推送发布版本
- **Supersedes**：本轮已修订[节点自动避让](../current/smart-canvas-node-auto-placement.md)、[选区整理](../current/smart-canvas-selection-arrangement.md)、[生成流程](../current/generation-pipeline.md)和 [Canvas Sync](../current/canvas-sync-implementation.md)；保留本规格用于验收追踪
- **Superseded by**：无
...
- 自动避让、同次多结果排列、不同次结果之间及选区整理统一使用代码中的唯一间距常量 **G = 4rem（64 世界单位）**；不增加画布或全应用配置 UI。整理删除原外框空间均摊，槽位直接使用 G。
...
两轮 Q1–Q12 均已由用户回答并合入正文；第 18 节无剩余产品决策。用户随后要求“开始实现”，已按确认后的目标交付代码。
...
实施前主路径的普通可见边缘安全距离为横向 200、纵向 96，同批内部间距为 48，主动整理下限为 32；这些是实施前不同代码层的基线，不是本规格的新默认值。前后端均存在旧间距判断，不能只统一前端常量。
...
| 一次运行的多个结果 | 新批次以实际输入父节点整体为来源；复用首项时以固定首项延续槽位 | 横向按视口换行，纵向按视口换列 | 首项原位保留；方向与视口冻结；新增槽位共用避让 |
| 再次生成（单图 / 多图 / 视频） | 被点击节点为布局来源 | 新建结果，旧节点不动、不复用 | 从点击节点右侧 G 开始，保留原参数与输入连接，共用避让和视口换行 / 换列 |
| 创建副本（菜单 / Cmd/Ctrl+D） | 原对象整体为来源；右侧 G、居中为初始偏好 | 刚性集合，保留内部相对位置 | 复用生成结果的避让、视口评分、Frame 扩容及自动竞争重试 |
| 粘贴、导入 Node Package | 用户明确目标 / 有效指针位置，或无位置时视口中心 | 刚性集合，保持内部连接和重叠 | 有真实明确位置则接受重叠；否则整体自动避让 |
...
flowchart TD
    ...
    C --> D{"需要重排内部对象？"}
    D -->|"否"| E["保持相对位置与内部重叠"]
    D -->|"是"| F["共用水平 / 垂直 / 宫格计算<br/>槽位间距固定 G = 4rem（64 世界单位）"]
    E --> G{"明确落点或用户主动整理？"}
    F --> G
    G -->|"是"| H["按锚点精确放置<br/>接受重叠"]
    G -->|"否"| I{"有来源？生成结果 / 创建副本"}
    I -->|"有"| I1{"再次生成？"}
    I1 -->|"是"| I2["以点击节点为布局来源<br/>保留原输入关系"]
    I2 --> J
    I1 -->|"否"| J["来源右侧；上游镜像到左侧<br/>比最近合法候选最多多移动 64<br/>优先改善整组视口可见性"]
    I -->|"无"| K["视口中心附近"]
    J --> L["生成批次按视口换行 / 换列<br/>固定首项作为槽位 0；使用 G 避让"]
    K --> L
    H --> M["得到最终节点坐标"]
    L --> M
    M --> N{"本次操作要求留在某 Frame？"}
...
主动整理明确进入精确布局分支，保持原锚点；复用旧节点作为首项时，该节点是固定对象，只有本次新增对象进入自动选位；生成批次按 R15 延续槽位。
...
**R04（Q12 已确认）**：无明确点时，有来源优先相对来源，无来源才使用当前视口中心。单来源使用来源外框，多选共同创建使用归一化来源整体外框。对执行节点 A 生成的新增结果，使用本次实际输入父节点的整体外框，不只取第一条上游，也不把历史 `generationBatchSourceNodeId` 的首项身份当成唯一父节点；没有外部父节点才以 A 为来源。例：`P → A`，A 原位复用并新增 B/C，输入来源仍为 P，但按后续确认的 R15 以 A 为固定首槽安排 B/C。多个实际父节点先归一化、去重并把编组成员投影为外部编组整体；来源解析沿用有效 Connection / 输入关系权威，不改变连线及引用顺序。
...
4. 使用操作发起时的 Canvas Viewport 快照；后续 Pan/Zoom、结果返回和其他编辑端视口不重新排布。生成批次先按 R15 换行 / 换列，再对各槽位应用相同距离预算及视口评分；完全可见比例固定为 1。视口仍容不下时允许超出，不缩小 Node、不移动首项。刚性集合保持内部几何。
...
**R10（Q4 / Q10 已确认，按后续指示调整）**：G 固定为 **4rem（64 世界单位）**，只存在于代码的统一常量定义，不新增 Canvas 字段、个人偏好、环境变量或设置 UI。前后端从同一权威定义读取 / 构建，不能再各自手写一个值。未来版本修改 G 只影响随后主动触发的布局，不重排旧坐标、不改变已创建 Pending Node。4rem 按画布 16px 基准换算为固定 64 世界单位，不随视口缩放或浏览器根字号变化。常量应保持有限正数。Quick Add 热区尺寸继续由 UI 定义，不用动态测得的热区反向改变 G。
...
**R28（按后续指示合并）**：菜单及 Cmd/Ctrl+D 的创建副本复用生成图片 / 视频的相对来源自动放置。副本来源是被复制的原节点（多选时为整体），不会因为继承外部输入连接而改用原节点的父节点；生成结果继续按 R04 解析实际父节点。副本保留内部相对布局，以 `source.right + G` 和 `source.y + (source.height - copy.height) / 2` 为初始偏好；方向、G 避让、视口预算、原直接 Frame 扩容及协作自动重试全部共用生成规则。最终位置允许因避让或视口调整偏离居中与恰好 G。删除副本独立的精确位置分支；已确认历史坐标仍精确恢复。普通粘贴、导入和 Alt 拖动继续使用各自的位置意图。
...
视觉行列仍使用既有边缘 / 中心跨度容差 `max(16 世界单位, 最小对应尺寸 / 2)`，槽位必须唯一、从左到右逐行连续填充，只有最后一行可以不满。这里 16 是识别容差，不是节点间距，不能随 G 的调整而扩大识别范围。
...
**R15（Q7 及后续换行确认）**：同次多结果按横向从左到右或纵向从上到下排列，方向在发起时冻结，间距 G。按发起时视口剩余宽度 / 高度及稳定槽位尺寸确定容量，横向放不下向下换行，纵向放不下向右换列。复用第一项时保留 ID、坐标，将其作为槽位 0 和障碍；其余节点围绕它安排，空间参照优先于重新靠近实际父节点，连接及输入来源不变。各新增槽位按稳定顺序避让已有节点及先前已安置的同批结果，共用 R06；有空间时保持紧凑网格，有障碍时允许必要偏移。首项在视口外或空间不足时仍允许超出，不移动旧节点或缩图。首项身份随操作意图持久化；竞争重试只重排新增节点，稳定槽位身份、一次创建和一次撤销不变。生成批次按各节点实际安全区校验，外框空位不是实心障碍；复制、粘贴和导入仍保持刚性集合外框。复用首项时只扩大首项原直接所属 Frame，不级联。

**再次生成（2026-09-06 修订）**：用户明确以点击节点为布局来源，覆盖此入口先前使用实际输入父节点整体的规则。新建结果从点击节点右侧 G 开始，共用自动避让和视口规则，多图继续换行 / 换列；旧节点不动、不复用。单图、多图、视频和失败后再次生成均使用该来源；输入连接及原生成参数不因空间来源变化而改写。
...
- 明确位置与自动位置必须在权威提交 / 重试中可区分；创建操作使用 `{node, placement}` 封装，重载保留元数据，公共 Node 与广播不携带视口；历史 inverse 精确恢复。边界见 [ADR-0011](../adr/0011-placement-intent-belongs-to-canvas-mutation.md)。
...
| A02 自动节点 / 同批 / 跨批 / 整理 / 服务端的间距常量 | 唯一 G = 4rem（64 世界单位） 和一致矩形间隔 | 已确认；前后端一起验证，无新增设置 |
...
| T04 | 来源右侧拥挤、父节点贴视口下边、父节点屏幕外 | 保持右侧与 G；在 dmin + 64 范围优先改善可见性，超过范围不跳远 |
| T05 | 多选来源、不同尺寸、跨 Frame / 同 Frame | 使用实际父节点整体外框；仅同一直接 Frame 可扩容，共同祖先不替代；输入顺序不变 |
| T06 | 无来源的上传 / 粘贴，Keyboard 创建 | 视口中心自动避让，不使用不存在的事件坐标 |
| T07 | 同次多结果横向 / 纵向，整批大于视口 | 内部顺序、G、方向快照一致；横向换行 / 纵向换列，空间仍不足才允许超出，不缩图 |
| T08 | 同来源续生成、旧批次远离父节点 | 新结果可打破旧起点对齐，返回父节点附近；原批次不移动，跨次间距使用 G |
| T09 | 空生成节点首次多图，有 / 无外部父节点 | 复用首项 ID 与坐标固定；新增结果围绕固定首槽排列，输入关系仍使用实际父节点；无复用首项时按实际父节点定位 |
| T10 | 相同几何分别自动创建、生成、主动整理 | 共享 G = 4rem（64 世界单位），前后端接受结果一致；整理不再按原外框均摊 |
| T11 | 水平 / 垂直顺序、可信四列两行、单行八节点、散点宫格 | 保留排序与宫格识别；只改变已选的间距机制 |
| T12 | 整理后撞到未选中节点 | 保留原整理锚点并接受重叠，不移动外部节点或整组选位 |
| T13 | 粘贴 / 导入含重叠、Frame 与 Connection 的集合 | 保留所有相对布局与连接；明确点精确放置，自动位置整体避让 |
...
| T27 | 在远离父节点的结果上点击再次生成，单图 / 多图 / 视频 | 从点击节点右侧 G 开始，新建结果，旧节点不动；输入仍来自原父节点，多图按视口换行 / 换列 |
| T26 | 单节点 / 多选 / 编组创建副本，高度不同、右侧占用、视口边缘或远端竞争 | 原对象来源，右侧 G 与居中为初始偏好；与生成结果共用避让和视口预算，保留内部关系并按相同规则重试 |
...
- 2026-09-06 再次生成来源修订：生产 Run → Output → Mutation → Placement 的回归先在旧实现落到 `(364,0)` 失败，改后按点击节点落到 `(764,200)`；35 项 Node 合同和 42 项聚焦 Python 回归通过。`tests/smart_canvas_regenerate_placement_browser_smoke.cjs` 通过真实右键菜单验证单图、四图换行、视频、旧节点原位、输入连接及服务端保存；只替换 Provider 提交，不调用真实生成。i18n、布局常量和 UI 版本检查通过。

- 换行修订复验：`node --test tests/smart_canvas_unified_layout.test.cjs tests/smart_canvas_multi_input_transactions.test.cjs` 34 项通过；定位 / Mutation / Persistence / 生成 / Realtime / 架构 114 项、后端协作 157 项、文档 / 缓存 / 交互 40 项通过。新增 `tests/smart_canvas_wrapped_generation_browser_smoke.cjs` 使用真实页面的生产 Generation Output 入口，验证四图固定首项、横向换行 / 纵向换列、可见性、服务端保存与整批 Undo/Redo；真实 Quick Add 鼠标拖动脚本也重新通过。页面测试不调用 Provider。完全可见候选固定评分为 1，避免浮点误差把合法槽位偏移。i18n 3464 键、布局常量和 UI 资产生成检查通过。

- Quick Add 实际手势回归：真实鼠标拖到 `(850,550)` 时，旧入口虽记录正确世界点，却因存在 `sourceTrigger` 被当成点击，创建到 `(414,297)`。现按 `drag.moved` 保存 `explicitPoint`，触发按钮仅负责菜单与焦点；真实拖动和菜单选择的输入 / 输出两侧 × 图片 / 视频 / 文本、平移 / 缩放、持久化及普通点击自动定位全部通过。新增 `tests/smart_canvas_drag_placement_browser_smoke.cjs`；30 项 Node 合同、36 项相关 Python 回归、i18n 3464 键与生成检查通过。
- **已解决：固定首项的四图布局。** 1300×800 视口，父节点 `(0,100,300,300)`，首项 `(364,100,400,300)` 固定；其余三项的刚性横排宽度为 1328，算法将其整体放到 `(364,464)`，最后一项落到 `x=1292`。这符合修订前 R15 的整行约束，却不符合用户希望四项接近并可见的体验；单独调候选评分不能让过宽的一行完整进入视口。用户已确认允许换行 / 换列，现由 R15 处理；横向坐标为 `(364,100)、(828,100)、(364,464)、(828,464)`，首项不动。

- 副本与生成结果合并复验：30 项 Node 合同、129 项几何 / Mutation / Persistence / 生成 / 整理 / 架构 / 文档 / 缓存回归通过；新增空闲、占用、视口边缘下同尺寸生成 / 副本位置一致，以及继承输入不改变定位来源、远端竞争自动重试覆盖。真实 Cmd+D 已验证右侧占用时避让并协作保存，页面 smoke 同轮通过；i18n 3385 键及布局 / UI 生成检查通过。保留此前居中要求为初始偏好，最终坐标由共用算法决定。

- 历史记录（已由 R28 合并修订覆盖）：创建副本补充复验：统一 Node 合同 28 项、Mutation / 定位 / Persistence / 架构 / 文档 / 缓存 50 项通过；真实页面 Cmd+D 在已占用的右侧 G 位置创建副本并通过协作保存，原有页面 smoke 同轮通过。接受重叠为按“刚好在右侧”的暂定解释，用户可进一步选择避让。

- 4rem 调整复验：核心几何 / 定位 / 整理 / Mutation / Persistence / Realtime 141 项、后端协作 154 项、文档 / 缓存 / 交互结构 39 项通过；统一 Node 合同 25 项和隔离真实页面重新通过。以下全域 Smart Canvas 记录属于调整前的整轮验证。

- `node --test tests/smart_canvas_unified_layout.test.cjs tests/smart_canvas_multi_input_transactions.test.cjs`：25 项通过，包括 G 权威、明确坐标、距离预算、实际父节点、编组投影、复用首项、失败回退、Frame 归属与重连元数据。
- Smart Canvas 全域 Python 回归：430 项中 429 项通过；仅 `test_reverse_prompt_dialog_is_created_lazily_from_the_formal_component` 的旧 `ic-cancel` 监听断言失败。已核对 HEAD 同样没有该监听，当前已用 `ic-hide`；与布局修改无关，不改动 Dialog 行为。其中统一布局 unittest 包装通过，自动执行新增合同测试。
- Canvas Realtime / WebSocket / Sync / Store / 生成幂等与文字恢复 / Presence：154 项通过，包含自动竞争、明确重叠、幂等、精确历史恢复、Frame 竞争和不兼容客户端拒绝。
- 文档地图、i18n 缓存、创建双语与布局合同：17 项通过。`validate-i18n.js` 最终共享资源检查 3443 个键通过（包含并行模型管理词条更新）；布局常量与 UI 资产版本 `--check` 通过。
- `smart_canvas_unified_layout_browser_smoke.cjs`：隔离 Workspace 的真实生产页面验证双向图片 / 视频 / 文字 Quick Add 锚点、Pointer / Keyboard 整理、Frame 扩容与整次 Undo/Redo、第二编辑端读取、刷新、中英文、Light/Dark、窄屏；无页面运行异常。该脚本不向 Provider 发起生成请求。

仍需人工产品体验复核（紧凑度与视口权衡、相邻热区手感、触摸等），以及合并后的发布检查；不将自动化结果表述为用户签收。旧画布、进行中的真实 Provider 运行与日常浏览器资料未用于本轮测试。完整端到端多客户端同时断线竞争仍可由后续发布验收扩充；服务端真实 WebSocket 竞争及客户端纯重试各自已有回归。

### 布局实现消融实验（2026-09-05）

以创建副本修正后的本地实现为基线，逐项删除并运行同一组行为合同；本轮只精简实现，不改变 R01–R28。没有增加布局开关、策略框架或新的持久字段。

| 实验 | 结果与决定 |
| --- | --- |
| 合并 Mutation 的规划、应用及转发函数；内联单一规划器读取 | 采用：保留一个 `canvasMutationPlanDrafts`，删除 3 个私有辅助函数；有限坐标验证由实际规划入口统一负责 |
| 删除 `placementMode`、未使用的 clone 偏移参数、未使用的成员矩形 / 索引字段 | 采用：查询统一通过已有 `placementIntent`；生产持久化本来已使用此接口 |
| 删除四方向相同的间距对象，以及非横纵批次的线性槽位计算 | 采用：直接使用 G / 2 和 G；刚性集合无需计算随后不用的槽位 |
| 删除视口距离边界候选 | 不采用：距离预算合同失败，无法保证最近合法距离加 G 范围内改善部分可见性 |
| 删除编组来源映射 | 不采用：编组成员作为来源的合同失败 |
| 删除原直接 Frame 扩容 | 不采用：原 Frame 及副本扩容的两项合同失败 |
| 用逐项最优选择代替完整候选排序 | 不采用：1,800 场景结果相同，但增加分支；5 次本地合成负载中位数仅由 137ms 到 132ms，不足以证明这项复杂度必要，保留可读排序 |

最终版本：28 项 Node 行为合同、129 项几何 / Mutation / Persistence / 生成 / 整理 / 架构 / 文档 / 缓存回归通过。固定种子 407 的 1,800 组场景（不同尺寸、障碍、方向、视口、锚点、批次与 Frame）逐项比较最终坐标、Frame 结果及几何，均与基线一致；这不是所有输入的形式证明。隔离真实页面的 Cmd+D、Quick Add、Pointer / Keyboard 整理、Frame、Undo/Redo、第二客户端、刷新及语言 / 主题 / 窄屏 smoke 通过。i18n 3444 键、两项生成检查和 diff 空白检查通过。实验临时变体已恢复，最终没有留下试验开关。

## 16. Rollout, migration and rollback

用户已授权并完成本地实施；尚未提交、推送或发布。不重排历史坐标。启动新版本前后端后，旧客户端需要刷新才能重连，连接携带 `layout_gap`，不兼容关闭码为 4410；创建时 G 不一致返回 `layout_contract_mismatch`。

旧 Canvas 坐标不迁移，不向旧文档补写 G 字段。G 随版本定义，新旧客户端的空间意图兼容与回退行为在协议设计中明确。只有新动作采用新合同。功能交付时按[完成文档规则](../agents/change-documentation.md)修订当前定位、选区整理、命中和必要的 Sync / 生成权威，核对 Issue 与对应 Gate。本地已完成 UI 资产版本检查；实际 Push 再执行项目发布版本规则。
...
| Issue | 已检索公开 open / closed Issues；[#34 优化树状整理](https://github.com/lazyq666/reroll-ai-canvas/issues/34)和 [#22 交互优化](https://github.com/lazyq666/reroll-ai-canvas/issues/22)为已关闭的相邻历史任务，不能冒称覆盖本次新合同。实施跟踪：[#40 统一智能画布节点定位、排列间距与 Frame 扩容](https://github.com/lazyq666/reroll-ai-canvas/issues/40)，In Progress |
...
剩余事项是人工体验复核与合并发布，不是未回答的产品选项。未来若实现发现新的真实冲突，应先把具体场景和备选结果带回评审，不恢复已消融规则作为隐含 fallback。
...
| 2026-09-05 | Implemented | 用户授权后实现统一 G、明确点、父节点与视口评分、固定首项、直接 Frame 扩容、刚性集合及操作元数据 / 精确历史恢复 | 第 15 节记录自动化、真实页面结果和人工待验收项；Issue #40 Review |
| 2026-09-05 | Implemented | 用户将 G 从 96 调整为 4rem（16px 基准下为 64 世界单位），统一常量、合同测试与当前文档同步更新 | 覆盖 Q10 原数值，其余共识不变 |
| 2026-09-05 | Implemented | 补充创建副本：原对象整体右侧 G、垂直居中，独立于无落点粘贴的视口规则 | 按用户“刚好在右侧”理解为明确相对位置，暂按接受重叠实施 |
| 2026-09-05 | Implemented | 完成布局实现消融：合并私有转发、删除重复查询与无用数据；保留经删除实验验证必要的视口、编组及 Frame 规则 | 第 15 节记录采用 / 不采用结果、1,800 组差分、28 项合同、129 项回归及真实页面验收 |
| 2026-09-05 | Implemented | 创建副本复用生成图片 / 视频的相对来源自动放置，删除精确副本分支；保留之前居中要求作为初始偏好 | 后续用户要求合并场景，覆盖此前接受重叠的暂定解释；避让、视口与自动重试共用 |
| 2026-09-05 | In Progress | 修复 Quick Add 拖动被触发按钮误判为点击；补真实鼠标 / 菜单验收。四图固定首项与其余整行布局的视口冲突已复现，尚待是否允许换行的规则决策 | 第 15 节记录红绿复验及未完成部分 |
| 2026-09-05 | Implemented | 用户确认后实现多图换行 / 换列、围绕固定首项、逐节点安全区校验和重试；修复完全可见比例的浮点误差 | R15 覆盖此前整行 / 整列限制 |
| 2026-09-06 | Implemented | 再次生成以点击节点为布局来源，单图 / 多图 / 视频共用；保留旧节点与原输入关系 | 覆盖该入口此前以输入父节点定位的规则 |
