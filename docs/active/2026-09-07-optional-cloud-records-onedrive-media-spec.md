...
- **Last verified**：2026-09-09（入口显示规则、画布编辑及常用生成操作消融；第二设备与另一网络仍待验收）。
...
生成前保存确认的负对照实验中，强制跳过确认后，尚未确认保存的目标节点到达模拟 Provider 提交入口，触发已有保护断言。因此保留该确认流程，也保留提交前后的编辑资格校验、权限检查、操作回执及断线恢复边界。
