...
每个进程取得远程编辑资格，服务器时间决定到期，期限 120 秒、每 20 秒续期，本机采用更早的保守截止时间。每个业务写事务开始和提交前都在同一远程写锁内检查资格。失败或未知续期撤销本进程资格；不会自动重试未知提交。相同设备可恢复自己的未完成任务；若有未完成 Run、outbox、publication 或批次，其他设备不能接管恢复。Provider 继续在本机执行。
...
- [运行时测试](../../tests/test_turso_runtime.py)：设备轮换、续期失败、原设备恢复及配置不匹配。
