...
普通生成结果以执行节点的**实际输入父节点整体**为来源，无外部父节点才用执行节点自身。**点击“再次生成”时以被点击的节点为布局来源**，新结果从它右侧 G 开始，旧节点不动、不复用；生成参数及输入连接仍沿用原规则。单图、视频和多图共用此来源，多图继续按视口换行 / 换列。复用固定首项时，输入关系仍来自实际父节点，但空间排列围绕首项，连接不变。

Canvas 的方向设置默认横向，在创建时冻结：横向从左到右，放不下时向下换行；纵向从上到下，放不下时向右换列。按发起时世界视口和稳定尺寸确定容量，槽位间距 G。空闲且完整可见的槽位保持原位；有障碍时按稳定顺序共用避让和视口评分，已安排节点加入后续障碍集。历史字段 `generationBatch*` 表达这组输出的布局标识；不改变 CONTEXT 中 Generation Batch 的持久任务集合定义。

同来源下一次结果可以候选续行或续列，但父节点距离与视口优先；允许打破跨次起点对齐，旧结果不移动。

空生成节点首次多图复用为第一项时保留 ID 和原坐标，将它作为槽位 0 和真实障碍，其余结果从下一个槽位继续。只扩大首项原直接所属 Frame。首项身份和原始视口随操作意图保存，并发重试沿用；整批仍一次创建、一次撤销。首项本来在视口外、空间不足或有障碍时，部分结果仍可超出，不移动旧节点或缩图。

生成批次按各节点实际安全区校验，外框空位不作为实心障碍；刚性复制 / 粘贴仍保持整体外框避让。完全可见的位置统一按 100% 评分，避免浮点误差造成无意义移动。
