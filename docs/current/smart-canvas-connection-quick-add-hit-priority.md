...
- 点击 Quick Add 以原加号为中心展开上、左上、右上三个圆形图标按钮，原加号切换为 ×，收起恢复 +。拖线松开以落点为中心，原节点保持 +；临时连线末端连续长成液态圆，中心直接显示 ×，随后展开同一组图标按钮。选择后在落点创建并连接节点；×、Escape 或空白关闭取消待创建连接并移除临时线。悬停或键盘聚焦向外侧显示说明。Gooey 动画仅绘制装饰背景；SVG 与按钮间空白不参与命中，菜单仍在 Top Layer。其定位、关闭和焦点合同见 [UI 设计与交互实现指南](ui-design-guidelines.md)。
