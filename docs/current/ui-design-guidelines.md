...
Layer Decomposition Node 的双击编辑态是 Image Studio 的专用业务变体，不是新的图片编辑模式：顶部不渲染 `ic-image-edit-mode-toolbar`，中央只显示透明背景上的图层合成结果，右侧浮动图层面板使用 `--ui-*` Token、`ic-badge`、`ic-icon-button` 与现有 Footer Dock。图层行默认优先呈现缩略图和名称，预览与删除动作只在 Pointer Hover 或 Keyboard Focus 时出现；预览动作切换显隐，删除动作移除图层。底部在 PSD 导出能力交付前只呈现禁用的“下载 PSD”按钮，不用普通图片下载制造错误预期。
