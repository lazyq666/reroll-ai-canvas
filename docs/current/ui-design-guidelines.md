...
Smart Canvas 在导航地图内部底栏显示实时缩放百分比和“重置视图 / Reset view”按钮。公共 `ic-smart-minimap` 通过 `controls` 属性和同名 Slot 承载控件，组件拥有底栏布局与事件隔离；控件点击和键盘操作不得触发地图导航。比例显示和重置行为由页面视口模块提供，中英文切换同步更新可访问名称，明暗主题使用公共 Token，窄屏保留底栏。

重置退出概览模式、清除旧概览视图及浏览器滚动偏移，并居中适配全部顶层节点；空画布恢复 45% 并居中世界原点。操作保留节点与选区，只保存个人视图，不提交 Canvas Mutation。滚轮和键盘缩放统一限制为 2%–800%，到边界保持缩放锚点且可立即反向操作；投影视图也执行同一范围保护。渲染前检查世界中心坐标；非有限值或绝对值超过个人视图允许的 10 亿世界单位时，将异常轴恢复至世界原点，避免将失效坐标交给 CSS 或空间查询。验收覆盖极端缩放恢复、空画布、重复重置、概览退出和节点内容不变，回归入口为 `tests/smart_canvas_viewport_reset_test.cjs`；`node tests/smart_canvas_viewport_reset_server.cjs` 提供使用模拟 API 的生产页面验收入口，覆盖 Pointer、Keyboard、中英文、明暗主题和窄屏。工作项为 [Issue #71](https://github.com/lazyq666/reroll-ai-canvas/issues/71)。
...
Prompt Generation Node 单选时打开独立的文字 Composer，与图片/视频 Composer 互斥。共用媒体 Composer 的 `composerCardTemplate`、5rem 指令行（含 20px 字符计数）、内边距和最大 48rem 宽度，适应视口并避让左侧 Dock；不显示重复标题、媒体模式、比例或数量。内容为可选引用缩略图、可折叠的只读上游文字、持续可编辑的指令；底部为模型和生成，展开位于共用外壳右上角。新布局与验收边界见 [Issue #47 规格](../active/2026-09-06-smart-canvas-prompt-generation-composer-spec.md)。
...
Smart Canvas Quick Add 的“引用该节点生成”浮层以 `ic-menu-reference-generate` 业务别名收录在“菜单、浮层与提示”，真实页面和组件库共同使用 `ic-menu[variant="reference-generate"]`。该变体使用下述 Quick Add Gooey 扇形图标布局，隐藏分组标签，保留文本、图片、视频三个命令项及可访问名称；组件库样板默认展开菜单本体，同时保留真实触发按钮以便关闭后再次操作。
...
“导航与命令”组件页按“视图切换、位置导航、内容翻页、内容操作、流程进度、状态”的任务顺序组织现有实例。分区导航必须与 Navigation Item、Expandable Navigation、Breadcrumb 相邻；Toolbar 与 Floating Toolbar 归入内容操作。Expandable Navigation（`ic-nav-disclosure`）直属的二级 Navigation Item 统一使用 `--ui-control-height-m` 固定行高，上下内边距计算在该高度内，不得额外撑高条目。分组标题与矩阵列名使用设计师可直接识别的名称，不把 `legal combinations`、`Complete states` 等合同术语作为可见标题，也不以用途说明、提示条或辅助说明卡补充分类。Tabs 的组件别名只描述可见的方向与内容形态，使用 `ic-tabs-horizontal`、`ic-tabs-horizontal-icon`、`ic-tabs-vertical`；Automatic / Manual Activation 仅保留为交互参数和内部合同维度。`horizontal-automatic-label` 组合的 Tabs Bar 使用 10px 圆角，每个 Tab Item 继续使用 `--ui-radius-s`（8px）圆角。所有 `ic-tabs` 变体的 Item Gap 默认统一为与 `ic-tabs-horizontal` 相同的 `0.125rem`（2px），`space` 参数可显式覆盖；`ic-tabs size="small"` 的 Item 字号固定为 `--ui-font-size-2`（12px），不得被调用页面的继承字号覆盖。Segmented Control 的纯文字与图标文字别名分别使用 `ic-segmented-control`、`ic-segmented-control-icon`；外层容器使用 10px 圆角，Item 使用 `--ui-radius-s`（8px）圆角。容器 Border 与选中项外描边均使用 `--ui-color-border-secondary`；选中项背景使用 `--ui-color-surface-segmented`，浅色主题映射到 `--ui-palette-gray-0`，深色主题映射到 `--ui-palette-gray-600`；选中项外描边不占用 Item 内部尺寸，Hover 不改变外观。Small / Medium / Large 容器高度分别使用 `--ui-control-height-s/m/l`，字号分别使用 `--ui-font-size-1/2/3`（10px / 12px / 14px），选项左右 Padding 分别为 `--ui-space-2` / `10px` / `--ui-space-3`。

Smart Canvas Composer 在普通编辑状态下，提示词区域（包含底部字数统计行）默认及最小高度为 `5rem`，最大高度为 `12rem`；随内容增减自动伸缩，清空后回到最小高度，超过上限时仅编辑内容区滚动。展开编辑继续填满专注编辑面板的可用空间，不受普通状态的高度上限约束。

公共 `ic-prompt-composer` 只允许普通文本和产品生成的引用 Token。粘贴文字时保留文字与换行，不带入外部字体、颜色、背景或其他富文本样式；插入保持当前光标、已有引用与浏览器撤销能力。剪贴板中的媒体文件继续交给页面上传流程。通过 `@` 插入引用后，光标位于其后的可编辑空格内，中文候选输入不得残留拼音或被编辑器提前中断。媒体与文字 Composer 共用此规则；真实页面回归见 `tests/laz_51_composer_browser_smoke.cjs`。

图片、视频生成结果的「继续编辑」（Continue editing）使用公共节点工具栏按钮，并在右键菜单提供同名入口。点击仅创建带有原次生成参数、继承原节点父级连接的独立空节点并聚焦 Composer；生成仍由用户单独提交。原结果 Composer 保留直接编辑能力。按钮只为具备生成信息的已完成结果显示，普通上传素材和空节点不显示。输入复制、连接和输出目标规则见[生成管线](generation-pipeline.md)。

Mention Picker 统一使用公共 `ic-mention-picker`。业务层只负责候选项筛选、引用状态与选择后的插入/引用；公共组件负责提示词行和媒体网格 Card、可选的媒体 `badge`、可选来源 Tabs、Loading / Empty / Error / Retry、容器锚定、默认固定 `18rem` 高度、方向键活动项、Pointer/Enter 选择、Escape 关闭与 Wheel 隔离。无候选项时 Loading、Error 与 Empty 必须互斥，优先级为 Loading 高于 Error、Error 高于 Empty；仅在已有候选项的分页加载中，Loading 才作为底部状态与内容并存。媒体模式的“当前画布 / 资产库”来源切换必须复用左对齐、内容宽度的 Small `ic-segmented-control`，不得拉伸占满容器剩余宽度。媒体 Card 目标宽度为 `5.625rem`（相对原 `3.75rem` 放大到 150%），默认 Border 使用 `--ui-color-border-secondary`；Card 外框、媒体容器与图片均使用 `--ui-radius-xs`。Hover 时 Border 切换为 `--ui-color-border-focus`、Shadow 切换为 `--ui-shadow-raised`，并显示从顶部 0% 不透明度渐变到底部 `--ui-color-mask` 60% 不透明度的 Mask。媒体区仅允许垂直滚动，图片与视频 Poster 使用现有最小档 `512px WebP` 缩略图，保持自然比例并以 `contain` 处理极端长宽比；视频只显示 Poster，不创建逐卡 Player；音频使用一个共享、手动启动且在移出/切换/关闭时停止的 Player。业务层每次按 60 项增量加载，已加载项必须累计保留；触底不得用下一页替换已有项，追加后必须保留滚动位置与活动项，反向滚动可访问所有已加载项。视觉方向键在媒体网格中选择相应方向的最近 Card，到达末尾时向业务层请求下一页。普通编辑状态下，调用方必须把 Composer 或节点容器作为几何 Anchor；全屏编辑状态下改用编辑内容区作为 Anchor，并选择 `overlay-block-end` 定位，使 Picker 与编辑内容区等宽、底边固定在底部操作栏上方并向上覆盖编辑区。靠近视口上边缘且不足以容纳 `18rem` 时，Picker 只缩小到 Anchor 上方的可用高度，第一项和滚动区域仍必须留在可视范围内。两种状态均可单独传入保留焦点的编辑器 Invoker；Picker 必须进入浏览器 Top Layer，高于全屏 Mask 与面板，且不得挤压编辑器布局。产品页面不得再拼接 Picker HTML、复制 Picker CSS 或维护独立定位帧。组件库必须把 `ic-mention-picker-prompt` 与 `ic-mention-picker-media` 作为同一公共组件的提示词和输入图变体并列展示；输入图样例必须使用真实来源 Tabs、图片/视频/音频媒体项和公共组件交互，不得复制 Smart Canvas 的 Picker DOM 或样式。

Mention Picker 媒体 Card 的 Hover Mask 内只显示名称；名称使用 `--ui-color-text-white`、`--ui-font-size-1`（10px）与 `--ui-font-weight-regular`，不得继承提示词模式的字号或 Bold 字重。Smart Canvas 中已经属于当前 Prompt Authoring 的媒体 Card 例外地把该名称替换为当前引用槽位的“媒体类型 + 序号”（如“图片1”“图片2”）。已引用 Card 必须先进入独立的 `.media-leading` 区域，复用 `ic-reference-thumbnail` 的方形封面、Border、圆角与底部通栏 Label 视觉；底部 Label 直接显示同一引用槽位名称，不再显示“已引用”，单卡固定为 `4.0625rem × 4.0625rem`（65px × 65px）。其余 Card 在自然高度的 `.media-columns` 网格中排列。两个区域均按从左到右、再从上到下的行优先顺序排列；不得由 CSS Columns 以从上到下的列优先顺序填充。媒体 Listbox 是唯一的纵向滚动容器；不得让固定高度的 Listbox 自身成为多列容器，以免卡片进入横向隐藏列、滚动条消失或无法反向滚动。

Composer 文本引用缩略图支持与图片相同的左右插入提示和拖拽排序，连接的文本节点、编组文本与本地 TXT 可在文本区域互相重排。文本统一复用 `ic-reference-thumbnail kind="text"`；`draggable="true"` 使文本缩略图可聚焦，本地 TXT 使用 `data-input-remove-local-text-reference` 并通过公共 `ic-remove.detail.localTextReferenceId` 移除。所有文本缩略图按可见顺序连续编号；媒体区域仍保持独立的图片 / 视频 / 音频引用顺序。文本缩略图可以聚焦，用 `Alt + ←/→` 向前 / 向后移动一项，移动后焦点跟随原引用；首尾越界不改变顺序。排序提示提供中英文，切换语言与 Light/Dark 不改写顺序和正文；预览、移除与单项引用状态继续可用。拖动取消或放回原位不保存变更，成功排序进入 Canvas 的保存和撤销/重做流程。最终用户提示词先按这份可见顺序拼接引用文本，最后追加 Composer 输入框中的手写正文。

Prompt Composer 内的媒体 `@` Token 不显示素材原始名称，必须显示与当前缩略图槽位一致的“媒体类型 + 序号”（如“图片1”“图片2”）。Token 高 20px，内嵌媒体预览高 14px；音频 Token 在圆形预览内绘制可见的静态波形，包含旧草稿恢复的 Token，不依赖运行时图标替换。文字与同一行正文在视觉上居中对齐，连续换行的 Token 边框之间保留可见间距。Token 自身不增加横向 Margin；Composer、Prompt Node 与 Prompt Generation Node 中相邻 Token 的视觉间距只由正文中的一个空格产生。缩略图拖动排序后，已有 Token 只按同一引用实例同步重编号，不改变正文中的位置；从缩略图移除某个引用实例时，Composer 中对应的 Token 也必须一并移除，同 URL 的其他引用实例不受影响。
...
`ic-segmented-control` 直接露出，不进入点击后浮层。参数区的 Model Picker 只接收业务层按 `image.layer_decomposition` 精确能力
筛选后的 Provider + Model，并显示两者身份。参数区提供智能识别与区域框选两个模式：智能识别使用与分层输出分辨率相同的 `ic-segmented-control`，直接展示四个预设，窄屏换行，下方始终显示可编辑输入框；不提供自定义选项，不显示粒度解释文字；自动判断默认留空，不自动附加提示词；区域框选支持编号多框、八向缩放、区域列表、像素数字调整、目标描述和自动提示词预览。模型、分辨率和参考价格共用，不叠加付费警告。
公共 Dialog 接收 `initialLayerDraft`，以 `layerDraft` 返回快照，通过 `ic-layer-draft-change` 交由宿主保存；关闭前复用可取消的 `ic-hide`，保存失败时宿主阻止关闭，显式放弃使用 `detail.reason: 'discard-layer-draft'`。关闭被取消后仍可继续编辑，确认关闭后才清理编辑监听。宿主拥有来源媒体身份和 Canvas Mutation。`supportsLayerRegions` 由业务层提供，未确认 bbox 语法的模型禁用框选提交。方向键和 Delete 只作用于获得焦点的图片编辑区，手势中 Escape 恢复本次手势。实现及尚待真实模型/触摸/协作验收的边界见[分层 Dialog 规格](../active/2026-09-05-smart-canvas-layer-decomposition-dialog-spec.md)。

智能分层结果节点在外侧显示名称 Badge，复用 `layers` 图标。单选智能分层节点时显示公共 `ic-smart-node-toolbar`，仅提供“预览”和“下载 PSD”：预览打开分层编辑器，下载 PSD 复用当前节点的导出流程；导出期间禁用当前下载按钮，结束或失败后恢复。节点预览、缩放约束与编辑器统一以分层清单的 `canvas_width / canvas_height` 为画幅权威；旧的图片预览尺寸或生成占位尺寸不得覆盖合成比例，重新打开已保存节点时同样适用。即使恢复记录的 `images` 包含底图和全部图层，也始终使用单个合成画幅，不能按图片数量转为多图网格布局。图层编辑器的合成预览在桌面编辑区水平居中；图层面板宽 `9rem`，圆角使用 `--ui-radius-m`。行操作默认不占布局宽度，在 Hover 或键盘 Focus 时浮现；图层列表独立滚动。底部工作栏依次放置下载 PSD、对比原图、缩小、缩放百分比、放大和适应窗口，复用图片预览控件；缩放范围为 5%–800%，100% 表示当前窗口下适配后的画幅，支持滚轮缩放、拖动平移与复位。对比原图使用可拖动分割线，键盘左右键与 Home/End 可调节位置；没有来源图片时禁用对比，图片加载失败时退出对比并允许重试。图层行仅提供显示／隐藏，不提供删除操作，避免误点移除生成素材；隐藏后可重新显示，图层数量和素材保持不变，PSD 导出保留隐藏图层及其显隐状态。显隐切换保留当前查看状态，关闭重开重置缩放、平移与对比。窄屏预览与面板上下排列，工作栏横向滚动以保持所有控件可访问。
...
- 图片 Node 的 `ic-smart-node-toolbar` 主栏依次显示生成图片/视频、智能分层、抠图、扩图、反推提示词、更多、分隔线、编辑和下载。「更多」复用小号 `ic-menu` 与 `ic-menu-item`，仅收纳角度控制、合成 GIF、灯光参考。入口与左侧「反推提示词」共用主栏按钮的字号、字色、图标尺寸、图标颜色和交互样式。节点操作栏及其外层容器按内容自然展开，不设置最大宽度，也不通过内部横向滚动隐藏命令；窗口窄于整栏时允许超出窗口。单击或按 Enter / Space 打开，方向键选择，Enter 执行；再次点击入口、点击外部、按 Escape 或选中命令后关闭。浮层跟随入口并避让窗口边缘，空间不足时独立滚动。切换节点、当前图片或语言时重建菜单并关闭旧浮层；图片移出可见画布时关闭浮层。专项命令继续使用打开菜单时的节点与图片序号，沿用原有处理、失败和重试流程；视频、音频、文本、生成中节点、编组与多选的动作保持各自规则。回归覆盖 `tests/test_smart_canvas_floating_ui.py` 和实际 Smart Canvas 页面。
...
- 插入正文的媒体 Mention Token 在 Composer、Prompt Node 与 Prompt Generation Node 中复用编辑器字号和行高；标签文字与同一行正文垂直对齐，正方形缩略图的边长比行高少 `2px`，使用 `--ui-radius-xs` 圆角，文字使用 `--ui-color-icon-tertiary`。Token 不显示背景或边框，复制生成中视频节点后仍保留可识别的缩略图与引用身份。
- `@` / `/` 快捷选择器在触发符后输入空格时必须立即关闭；之后输入的新 `@` 或 `/` 必须开启独立的空查询会话，不得把前一个触发符识别为查询来源。
- 远端协作状态不能覆盖本地正在输入或拖动的临时状态；确认、冲突与重同步必须让用户知道结果。

### Smart Canvas 动效：Quick Add、提交与生成 Badge

- Quick Add 的 `ic-menu[variant="reference-generate"]` 点击入口以原加号为中心展开三个 44px 圆形图标按钮：图片在上方、文本在左上、视频在右上，对应中心偏移 (0,-64)、(-54,-34)、(54,-34) CSS px；展开时原加号切换为 ×，其位置由 Top Layer 中的圆形关闭按钮承接，点击收起后恢复 +。靠近视口边缘时选择向下、向左或向右展开，再进行边界约束；点击加号通过 `show(trigger)` 锚定真实按钮；拖线到空白处松开通过 `showAt(x,y,trigger)` 锚定松开坐标，原节点保持 +，落点中心直接显示 ×，不播放 + / × 旋转。拖放先在保留的临时连线末端用 120ms 将小圆长成完整菜单中心，并从连线蓝色过渡到菜单背景，再开始相同的液态分离；选择后在落点创建并连接节点，×、Escape 或外部关闭取消待创建连接并清理临时线。减少动态效果直接显示最终按钮。两种入口共用此菜单，覆盖下游生成、上游输入和多输入（多输入保留已有图片/视频选项）。进入时三个完整的圆从已解析的屏幕锚点粘连、拉伸后分离，退出时重新融合；不得用圆从小放大替代液态分离。背景、边框与阴影绘制在共用 SVG 轮廓上，独立按钮不得绘制截断液态连接的底色或边框。图标延迟显现，不参与轮廓滤镜；进入中的自动 Focus Ring 暂缓至按钮静止后显示。静止后保留同一 SVG 轮廓且不调度动画帧，避免切换原生按钮背景时闪变；按钮间空白不拦截画布命中，不出现矩形菜单底板。三个生成选项的 Hint 分别为“生成文本 / Generate text”“生成图片 / Generate image”“生成视频 / Generate video”。菜单项保留命令事件与可访问名称，`appearance="icon"` 使用 `hint`（未提供时取 `label`）显示朝扇形外侧的公共 Tooltip，避免遮挡关闭按钮；`close-label` 提供关闭按钮的双语可访问名称。Pointer 悬停和键盘聚焦均可读说明，支持中英文、左右/上下方向键、Home/End、Enter、Escape、外部关闭和 Focus 返回。该专用变体沿用参考 PlusMenu 的 550ms `cubic-bezier(.34,1.56,.64,1)` 展开、40ms 错峰；图标延迟 120ms 加错峰后在 180ms 内显现。收起在 250ms 内以 `cubic-bezier(.22,1,.36,1)` 融合，共用轮廓与关闭按钮保持最多 5px 的同步回弹。点击入口的关闭圆在融合尾段连续缩回原 Quick Add 的实际尺寸与位置，× 旋转回 +；250–450ms 将轮廓、图标与真实按钮交叉淡化，避免 44px 菜单圆直接切到原按钮的尺寸、边框和阴影。拖放入口没有需要承接的落点按钮，关闭圆和 × 在同一 250ms 融合期间一起淡出，保持 ×，不残留额外的 + 或等待更长的回弹后硬切移除。动画取消或断连必须恢复原按钮透明度。滤镜使用 6px Blur 和 18/-7 Alpha 阈值；减少动态效果直接展示原生按钮；退出即禁止交互，快速重开、滚动/窗口/主题变化和断连清理旧动画及提示。边缘翻转与画布缩放继续由菜单定位合同处理。
- Composer 的主提交 `ic-icon-button` 显式设置 `effect="metal"`，由 Actions 家族原生适配 Metal 的金属轮廓与光晕；其他 Icon Button 默认不启用。装饰位于 Shadow DOM，`aria-hidden`、不参与命中，整个装饰层（含 Canvas 与外发光）在原生按钮内部按圆形裁切，并跟随按压缩放；任何装饰像素不得越过当前按钮圆框。现有 Pressed / Focus、无障碍名称、提交中 Spinner 和防重复提交继续有效；Disabled、Loading 与减少动态效果时隐藏装饰，离屏/后台时暂停绘制。WebGL2 不可用或初始化失败时保留普通按钮；断连释放实例和监听器，重连可恢复。主题跟随 `data-ui-theme`，无需 React 或运行时外部服务。 绘制目标为 15 FPS，细圆环按设备 DPR 的两倍采样（上限 4），共享 Shader 分辨率不变。
- `ic-generation-pending` 的文本、图片、视频在 `queued` 和 `generating` 均使用同一个 20px Thinking Orbs `breathing` 点阵环；`ic-badge-node-runtime-status` 通过 `ic-badge[kind="status"][loading][loading-animation="orb"]` 使用完全相同的绘制预设。排队/生成含义由文案区分，不推断服务端未报告的进度。完成后移除动画并保留原来的静态耗时展示。更新耗时不重建 Orb Canvas，Halftone 占位内容保持原规则。Pending 复用原 Halftone 时钟，独立运行 Badge 共用一个最高 24 FPS 时钟；离屏/后台暂停、断连释放，系统或项目减少动态效果显示同一静态环。Canvas 2D 不可用时回退默认加载图标。公共 Badge 的 `loading-animation` 可为 `spinner`（默认）或 `orb`，`indicator` Slot 允许消费组件提供装饰图标。
- 三种效果随明暗主题变化，状态文案继续通过现有中英文 i18n 资源生成。依赖固定在 `static/vendor/libraries-motion/999866f/`，保留 MIT、Paper Shaders 的 Apache-2.0 LICENSE 与 NOTICE；原始引擎源码与 `npm run build:motion` 构建入口一同维护。

验收入口：`tests/libraries_motion_browser_smoke.cjs`（真实 Smart Canvas、假 Provider 回执、24 实例绘制与暂停/释放）、`tests/generation_pending_badge_browser_smoke.cjs`、`tests/generation_orb_browser_smoke.cjs`（统一环与运行 Badge 生命周期）、`tests/metal_quality_browser_smoke.cjs`（DPR 1/2 的采样与刷新率）、`tests/issue_187_quick_add_frame_drop_browser_smoke.cjs`（拖线落点连续展开、源加号保持、直接 ×、真实图标创建节点与三种取消方式）、`tests/composer_submission_feedback_browser_smoke.cjs`、`tests/menu_popover_browser_smoke.cjs`、`tests/gooey_liquid_browser_smoke.cjs`（完整圆、几何分离后的连接像素、弹性超调、轮廓/Focus 分层、整体收起回弹与零空闲动画帧；`GOOEY_THEME=light` 验证浅色）。本地浏览器测试服务使用 `SMART_CANVAS_PORT=8794 python3 tests/smart_canvas_manual_server.py`；动效测试读取 `SMART_CANVAS_BASE_URL`。真实 Provider 请求、Safari/Firefox 和低端 GPU 的表现仍需对应环境验证。
...
Image Node、Generation Node 与 Prompt Generation Node 均提供稳定失败态，沿用生成目标形态展示“生成失败”、失败原因与“查看日志”动作；这些失败态的实体外壳统一使用 `--ui-color-surface`，Image Node 不得继续继承媒体就绪态的透明外壳。失败图标容器为 `2rem × 2rem`，不显示 Border、背景或 Shadow；Error 图标使用与默认 Zap 相同的 Medium 尺寸。组件库行标签与首个 Node 保持至少 80px 的视觉间距，Image Node 使用项目自有的几何测试素材 `static/images/test/fixture.svg`，Prompt Generation Node 同时展示含上游 Image 的输入状态。除无实体外壳的 Annotation 外，Node Host 选中时统一使用 `--ui-color-border-focus` Border 与 `--ui-shadow-overlay` Shadow，不由内部媒体或业务容器代替选中反馈，Image、Video 与 Audio 内容也遵循同一规则。Smart Group 与 Frame 使用与其他实体 Node 一致的 `12px` 圆角，Smart Group 默认使用 `--ui-shadow-raised`；远景模式中的名称 Tag 继续分别表达 Smart Group 的 Surface/Border/Shadow 与 Frame 的框架色，而不是合并成无差别标签。上传 Node 只允许共享主按钮通过 Pointer 或 Keyboard 打开文件窗口，空白区域仍可接收 Drop；标题为“拖拽、粘贴或点击选择文件”，说明为“支持图片、视频和音频，最大 500MB”，对应产品实际接受的媒体类型和服务端单文件上限。外层 Surface 不重复绘制，图标容器使用 `--ui-color-surface` 背景、`--ui-color-border-tertiary` Border 与 `--ui-shadow-raised` Shadow；除上传按钮外，Upload Node 主体均可拖动 Node，Generation Node 除“查看日志”等操作按钮外也遵循相同规则。Prompt Node 与 Prompt Generation Node 的展开动作位于选中态 `smart-node-floating-menu`；文字 Composer 右上角也提供展开/收起动作。普通 Prompt 全屏继续使用公共 Focus Surface 的 Escape / 外部点击关闭。Video Node 加载或重绘时不自动播放；Pointer 悬停在节点上即从该媒体实例保存的进度播放，离开节点即暂停并停留在当前画面，不重置进度。选中节点本身不改变播放状态；进入应用内全屏后立即播放。Video 与 Audio 在当前 Canvas 会话内共享一个活动媒体名额；切换到另一个 Video 时暂停前一个并从目标自身保存的进度播放，Audio 仍由显式媒体操作启动。Video 不因失去单选、进入多选或取消选择而恢复封面；悬停播放和全屏播放按 Node 媒体实例保留进度，同一 URL 的不同实例不共享进度。视频节点内的播放态使用公共 `ic-media-player-controls variant="node"`，不保留封面中央播放图标；新 Video 媒体实例默认开启自动循环，Node 的 `smart-node-floating-menu` 与节点/应用内全屏播放器共享同一个“自动循环”Toggle 状态，切换 Toggle 本身不触发播放。节点浮动菜单的开启态使用黑色背景、Check 图标与“循环已开启”文案，关闭态恢复普通背景、Loop 图标与“自动循环”文案；播放器内使用前述循环图标按钮；关闭循环后自然结束停在末帧，下一次播放从 0 开始。节点播放器与应用内全屏播放器均不提供浏览器原生系统全屏、画中画或投送入口；双击 Video 媒体与 `smart-node-floating-menu` 的“全屏播放”执行同一应用内全屏动作，以保持画布定义的媒体比例与完整显示。节点与应用内全屏复用同一个 Video 媒体元素，只改变展示位置和尺寸。进入全屏立即从当前进度播放，不重新加载媒体；音量、静音、播放速度与循环状态连续保留，且任一时刻只允许一处播放。退出全屏时暂停并保留当前画面与进度；再次悬停节点时继续播放，返回节点后画面与声音来自同一播放器。全屏期间的画布重绘不得替换该播放器或创建后台播放副本。切换预览媒体时暂停上一实例；关闭后不得由过期加载回调重新启动隐藏或已移除的媒体。进入 Composer 展开/全屏编辑或普通 Prompt 全屏编辑、打开普通 Dialog、页面失焦、标签页隐藏、最小化或离开页面时暂停活动媒体，关闭或返回后不自动恢复；播放失败必须保留明确重试入口，删除媒体实例时清除其会话状态并关闭正在展示该实例的应用内全屏。
...
Smart Canvas 的媒体文件名 Badge 在文件名前显示媒体身份：图标按实际内容区分图片、视频或音频，文字只表达来源。上传媒体统一显示“已导入 / Imported”，Generation Output 统一显示“AI 生成 / AI generated”；已完成结果不得因 Composer 后续模式变化而改写身份。单媒体、多媒体与 Smart Group 中的媒体使用同一规则，优先保留媒体自身的生成身份；历史记录只在有明确生成输出证据时使用生成名称，其余沿用上传素材分类。身份与原文件名并列显示并在空间不足时省略，Hover Title 保留全文。双击文件名或右键命中的单个媒体均在 Badge 原位编辑名称主体并全选原文；编辑期间保留左侧图标和媒体身份，仅名称切换为无描边的白色输入区，Badge 不下移，输入区不覆盖图片，浮动节点工具栏位于 Badge 上方；Enter 或单击画布其他非输入区域提交，Escape 取消，提交后保留实际媒体扩展名。Frame 标题采用相同手势。媒体列表在编辑期间重排时仍按稳定身份修改原目标；目标失效或不唯一时不得按旧索引误改其他素材。下载优先使用保存后的媒体名称，没有名称才回退 URL 文件名，并始终补齐一个真实扩展名。完整合同见 [Smart Canvas 媒体命名与重命名](../active/2026-09-20-smart-canvas-media-naming.md)。
...
图片节点的「合成 GIF」复用 Large `ic-ai-processor-dialog`，使用 `processor="grid-gif"`。左侧原图显示完整角色、归属折线、独立特效彩框与对齐基准；右侧包含素材排列、边界／脚底调整方式、播放速度与背景设置；控制点仅在画面中使用鼠标点击选中、拖动调整，不提供键盘快捷键和边界／对齐操作提示，不提供调整点下拉框和坐标输入。「调整方式」「GIF 播放速度」「GIF 背景」统一复用 Medium `ic-segmented-control`，标题在上，选项直接露出并左对齐；选项使用简短中英文，速度悬停说明保留每帧时长，窄屏不得截断选项；选择「透明」后用棋盘格预览并隐藏颜色选择器，切回「纯色」后恢复原颜色，棋盘格不导出。横、竖折线默认沿透明间隙自动避让角色与独立特效，可拖动控制点修正，或使用「重新自动分组与对齐」恢复自动结果；尚有素材跨越边界时明确提示。折线只决定独立特效归属，不裁切相连的角色与武器；身体中心和脚底基准可用鼠标拖动修正。窄屏上下排列，复用共享 Select、Segmented Control、Number Input 与 Color Field。详见[宫格图片合成 GIF](../active/2026-09-10-grid-gif.md)。
