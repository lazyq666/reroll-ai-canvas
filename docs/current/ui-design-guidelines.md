...
Halftone 圆点尺寸仍以 `dot:18%` 参与场值变化，但正式 `ic-generation-pending` 与动画实验 B 都使用 `2px` 最小圆点半径，避免常用缩放范围内的小圆点因抗锯齿失去实色核心。`ic-generation-pending` 外置 Status Badge 使用 `--ui-font-size-2`（12px）字号和 `--ui-font-weight-regular` 字重，不使用加粗样式；耗时属性更新必须保留现有 Badge 与 Spinner 实例。
