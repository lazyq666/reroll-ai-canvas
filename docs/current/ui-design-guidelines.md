...
Prompt Generation Node 单选时打开独立的文字 Composer，与图片/视频 Composer 互斥。默认宽度最大 36rem，适应视口并避让左侧 Dock；不显示重复标题、媒体模式、比例或数量。内容为可选引用缩略图、可折叠的只读上游文字、持续可编辑的指令；底部仅模型、展开和生成。新布局与验收边界见 [Issue #47 规格](../active/2026-09-06-smart-canvas-prompt-generation-composer-spec.md)。
...
普通 Prompt Node 的展开编辑使用 Nodes 家族公共 `ic-prompt-node-focus-surface`。组件拥有遮罩、`850 × 660px` 最大面板尺寸、16px 小视口安全边距、Modal 语义以及点击面板外区域与 `Escape` 请求收起的交互；展开面板不显示收起按钮。Smart Canvas 负责普通 Prompt Node 的可展开资格、正文、编辑持久化、子浮层优先级、开关状态与收起后的焦点返回。Prompt Generation Node 使用专属文字 Composer，展开时将同一个编辑 DOM 移入 Large `ic-dialog`，保留指令、光标与滚动位置；展开/收起动作始终位于其底部操作区。
