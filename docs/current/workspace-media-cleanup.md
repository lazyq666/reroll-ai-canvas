...
读取失败、无法完整解析的 JSON/SQLite、未知记录类型、数据目录软链接或缺失权威数据库会使
...
- JSON 记录按完整文档读取，单文件上限为 256 MiB，兼容含内嵌媒体的大型迁移备份；
  超过上限仍中止扫描，不只读取前半段。
- `data/recovery/` 内的 `.json.bak` 按 JSON 读取。对于历史
  `*.corrupt-YYYYMMDD-HHMMSS.json.bak`，只有错误发生在文件末尾、字符串均已闭合、
  且仅补齐缺少的对象/数组结束括号就能完整解析时，允许在内存中补齐并收集全部引用；
  不改写备份，不补造字段或值。截断字符串、缺值、尾逗号和中途损坏仍阻止清理。
- 旧 `data/models/matting/` 中已知的 `birefnet-general.onnx` 与
  `birefnet-general-lite.onnx` 普通模型缓存文件不参与引用检查，也不删除。
  其他未知文件和软链接仍阻止清理；不跳过整个恢复目录或模型目录。
...
旧布局回归另覆盖完整/仅缺结束括号的恢复备份引用保留、其他损坏备份拒绝、
已知模型缓存兼容与未知/软链接拒绝、超过旧 64 MiB 上限后的引用保留，
以及超过 256 MiB 时停止清理。
