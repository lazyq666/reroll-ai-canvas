...
管理员可在“数据存储位置”中手动扫描并清理无引用的 Managed Media。普通内容删除
只解除引用；仍被画布、有效撤销历史、生成历史或资产库等内容引用的原文件继续保留。
扫描、确认与临时保护的完整合同见[工作区未使用文件清理](workspace-media-cleanup.md)，
生命周期边界见 [ADR-0012](../adr/0012-manual-workspace-media-cleanup.md)。
...
云端设置仅管理员可见，默认隐藏。需要显示入口时，在项目根目录的隐藏文件 `.env`
中加入 `INFINITE_CANVAS_SHOW_CLOUD_RECORDS=1`，退出应用并通过统一启动器重新启动。
此配置只作用于本机这份安装；`0`、未配置或无效值均按隐藏处理，仅 `1` 表示显示。
它只控制入口，不配置连接、不导入数据，也不启用或关闭云端保存。已启用云端的当前
工作区始终显示入口，包括连接中断时；状态沿用按 `storage-authority.json` 启动的云端
运行时，不以是否连通判断是否启用。关闭云端并成功回到本地、重启后，入口再次按本机
配置显示或隐藏。迁移核验、本机管理员操作限制和画布保存错误提示保持有效。

启动时云端身份或格式检查失败就停止；不会自动启用旧本地 SQLite。若编辑资格仍被占用，
在 125 秒窗口内每 2 秒重试获取资格，让上次退出后未释放的记录自然过期；等待期间保持
启动中，取得资格后自动继续。持续占用超过等待窗口则停止，提示退出已打开的 Reroll、
等待 2 分钟后重启。占用提示不表示一定有另一台设备在线。网络失败、无效绑定直接停止。
每个云端业务事务检查远程资格，续期失败立即停止写入。暂时的连接错误可在本机截止时间内
重新核对并续期同一未过期资格；确认后恢复新连接，旧连接保持失效。资格过期、被接管或
绑定改变时不自动恢复，需恢复网络后通过启动器重启。换设备前完成或取消任务并退出应用，
