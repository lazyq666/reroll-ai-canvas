...
- Status: Accepted
- Date: 2026-09-04
- Amended: 2026-09-05：移除独立资料刷新，只保留模型拉取中的能力提取；外部 AI 查找与能力数据导入亦已移除，保留人工详情维护。
- Amended: 2026-09-07：移除费用词汇及字段名片段拦截，保留有明确合同含义的结构、状态、上下限与发布保护。
...
2026-09-05 最终补充决定（取代早期外部研究包与 AI 初始值方案）：移除复制查找要求、能力包导入、导入预览与应用 API，以及为其生成的研究线索和 Schema。用户试用后认为外部 AI 结果不理想；格式校验与人工覆盖保护无法解决资料质量和当前表单的表达限制，继续维护该流程的收益不足。能力缺口由 Administrator 在模型详情中人工维护；API 拉取模型中的确定性资料提取仍只产生 Evidence / Draft。历史导入已产生的 Published、Evidence 与审计记录继续保留，读取不会改写数据，管理员仍可通过详情人工修正。新人工保存继续记录 manual 来源，不再维护导入专用保护状态。
...
详情接口中的 `video` 对象只是面向产品表单的读写投影，不是第二份持久化能力合同。保存时直接更新上述统一合同和既有视频命令合同；Composer 直接消费命令合同，不再从中间“视频档案”重建运行参数。
...
产品不提供价格、计费、额度或消耗配置，本决定也不新增此类参数。运行时不通过关键词或字段名片段判断“是否涉及费用”：此类检查曾误伤版本哈希中的 `fee` 和合法字段名 `feedback`，不能证明合同有效。自由资料文本按原文保存；能力合同继续按结构、支持状态、参数类型和数量边界校验。新增产品字段仍须遵循本目录的生成约束职责。
...
- **在 Reroll 内置 AI 搜索与填表**：可以减少一次复制粘贴，但会把联网研究、模型选择、AI 额度和失败状态带入管理员产品；该执行器与后续尝试的外部研究包导入均已移除，采用模型拉取资料提取与人工详情维护。
...
- 不再提供外部研究包导入；既有导入历史与已保存能力继续可读、可人工修改，不因入口移除而清除。
...
- 每个 Draft 已填写的叶子字段必须绑定同一 `Provider + Model + Model Operation` 身份下的 Evidence 和置信度；矛盾上下限或合同外字段不能进入审核。
