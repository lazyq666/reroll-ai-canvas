...
- Linear 是需求、Bug、调查和开发任务的跟踪位置；仓库不再维护重复 `roadmap.md`。
- 临时 Handoff 不作为长期文档类型。仍在进行的交接应放在对应的 Linear 任务或 PR；稳定规则进入 Spec，提交和分支上下文留在 Git。
...
需求、Bug、调查和开发任务在 Linear 中跟踪；已有 GitHub Issues 保留为历史参考，新工作无需创建 GitHub Issue。`docs/active/` 只保存需要长期评审的功能规格，不保存任务板镜像、跨电脑分支指令或临时聊天交接。
